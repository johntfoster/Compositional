...
"""
    ...
    def test_inventory_scans_input_templates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            include_root = root / "includes"
            scan_root = root / "templates"
            include_root.mkdir()
            scan_root.mkdir()
            template = scan_root / "example.i.template"
            template.write_text(
                "[Kernels]\n  [template_object]\n    type = ADReaction\n  []\n[]\n",
                encoding="utf-8",
            )
            ...
            catalog.write_text(
                yaml.safe_dump(
                    {"schema_version": 1, "include_root": "includes", "fragments": []},
                    sort_keys=False,
                ),
                encoding="utf-8",
            )
            registry.write_text(
                yaml.safe_dump({"schema_version": 1, "blocks": []}, sort_keys=False),
                encoding="utf-8",
            )
            with mock.patch.object(verified_blocks, "ROOT", root):
                report = verified_blocks.discover_inventory(catalog, registry, [scan_root])
        self.assertEqual([item["path"] for item in report["files"]], ["templates/example.i.template"])
        self.assertEqual(report["files"][0]["objects"][0]["type"], "ADReaction")
