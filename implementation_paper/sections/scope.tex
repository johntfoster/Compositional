...
\begin{itemize}[leftmargin=*]
	...
	\(\eta_\xi^\alpha\);
	\item thermodynamic and field forces: \(\bar p_f\),
	\(\hat\mu_\xi^\alpha\), \(\mu_\xi^\alpha\), \(\bar p_E\), \(\gamma_f\),
	\(\tau\), \(B\),
	\(\mbf E\), and \(\mbf d_f\), with explicitly recorded held-fixed
	variables for each derivative, together with algebraic recovery of
	\(L_\xi^\alpha\) from \(\hat\mu_\xi^\alpha\), \(\tau\), and the phase
	kinematics;
	\item flux and stress closures: \(\mbf w_f\), \(\mbf W_f\),
	the conversion-corrected mobilities
	\(\widetilde{\bs\lambda}_f\) and \(\widetilde{\bs\Lambda}_f\),
	\(\mbf P^{\prime\prime}\), and the pressure stress contribution
	\(B\bar p_EJ\mbf F^{-T}\);
	\item conversion closures: \(\dot r_{(m)}\), \(\dot c_\xi^\alpha\), and
	the traditional electrochemical affinity and transfer-work correction used
...
solid-reference component storage variables, phase partition variables, the
single transfer potential \(\tau\), and the phase-reference transfer-work
levels required by the affinity law.  A thermodynamic material
maps those variables and \(\mbf F\) to
\(\phi_\xi\), \(\eta_\xi^\alpha\), \(\bar p_f\),
\(\hat\mu_\xi^\alpha\), \(\mu_\xi^\alpha\), \(\bar p_E\), \(B\), and
\(\dot c_\xi^\alpha\), while the \(L_\xi^\alpha\) are recovered
algebraically.  This first problem adopts the
neutral, nonelectrical single-fluid specialization
\(\gamma_f=0\), \(\mbf E=\mbf d_f=\mbf0\), while retaining the transfer
potential \(\tau\) in the conversion-insertion force.  The first kernels
therefore include the component storage residuals, the solid storage residual,
the single \(\tau\) evolution residual, the algebraic transfer-work recovery,
the reference-flux closure, and the quasi-static skeleton momentum residual.
Additional multifluid phase
equilibrium, electrical coupling, thermal effects, and plasticity are planned
extensions.  A later charged nonisothermal extension must use the
gauge-invariant relative-transport force
\(\theta_{\mc G}\nabla[(\hat\mu_\xi^\alpha-\hat\mu_\xi^N)/\theta_{\mc G}]
+(z_\xi^\alpha-z_\xi^N)\nabla\varphi\) and include the electric
field work on the relative charge flux directly in the energy residual.  It
must not add an isolated \(\varphi\)-weighted charge flux without the associated
charge-storage and source terms.  Its general two-temperature reaction residual
must use
\(-\sum_{\mc G}\theta_{\mc G}^{-1}
\sum_{\xi\in\mc G,\alpha}(\psi_\xi+L_\xi^\alpha)\nu_{\xi(m)}^\alpha\);
electrochemical coefficients are interchangeable only when charge is conserved
separately in each thermal subsystem.  Any coupling to reaction-energy transfer
must preserve the mechanismwise dissipation condition in the theory.
