#include "ADHydrostaticBarotropicSkeletonStressMaterial.h"

#include "metaphysicl/raw_type.h"

registerMooseObject("MulticomponentReactiveFlowApp",
                    ADHydrostaticBarotropicSkeletonStressMaterial);

InputParameters
ADHydrostaticBarotropicSkeletonStressMaterial::validParams()
{
  InputParameters params = Material::validParams();
  params.addClassDescription(
      "Computes the pressure-coupled double-prime effective first Piola stress generated "
      "directly by a hydrostatic double-prime potential with mineral-density EOS "
  ...
  params.addRequiredRangeCheckedParam<Real>(
      "shear_modulus", "shear_modulus>0", "Skeleton shear modulus.");
  params.addRequiredParam<Real>("lame_lambda", "Skeleton first Lame parameter.");
  params.addRequiredRangeCheckedParam<Real>(
  ...
  params.addParam<MaterialPropertyName>("effective_first_piola_name",
                                        ...
                                        "Output pressure-coupled effective first Piola stress.");
...
ADHydrostaticBarotropicSkeletonStressMaterial::
  ...
  : Material(parameters),
    ...
    _mineral_effective_pressure(declareADProperty<Real>(
        getParam<MaterialPropertyName>("mineral_effective_pressure_name"))),
    _mineral_effective_pressure_jacobian_derivative(declareADProperty<Real>(getParam<
        MaterialPropertyName>("mineral_effective_pressure_jacobian_derivative_name")))
{
}

void
ADHydrostaticBarotropicSkeletonStressMaterial::computeQpProperties()
{
  ...
  const ADReal numerator =
      _shear_modulus * (pow(J, 2.0 / 3.0) - 1.0) + _lame_lambda * log_J;
  const ADReal numerator_jacobian_derivative =
      (2.0 / 3.0) * _shear_modulus * pow(J, -1.0 / 3.0) + _lame_lambda / J;
  const ADReal q = -numerator / (_reference_solid_volume_fraction * J);
  const ADReal q_jacobian =
      -(numerator_jacobian_derivative * J - numerator) /
      (_reference_solid_volume_fraction * J * J);
  ...
  const ADRankTwoTensor skeleton_first_piola =
      _shear_modulus * (_F[_qp] - F_inv_T) + _lame_lambda * log_J * F_inv_T;

  const ADReal pressure =
      _equivalent_pressure[_qp] + (_equivalent_pressure_enrichment
                                        ? (*_equivalent_pressure_enrichment)[_qp]
                                        : 0.0);
