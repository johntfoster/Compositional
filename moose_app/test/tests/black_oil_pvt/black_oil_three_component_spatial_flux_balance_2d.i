...
[Functions]
  ...
  [water_saturation_exact]
    type = ParsedFunction
    expression = '0.2+0.05*x+0.04*y'
  []
  [gas_saturation_exact]
    type = ParsedFunction
    expression = '0.2+0.03*x+0.02*y'
  []
  [porosity_exact]
    type = ParsedFunction
    expression = '0.25'
  ...
  [water_pressure_exact]
    type = ParsedFunction
    expression = '1.1+0.9*x+1.92*y'
  []
  [gas_pressure_exact]
    type = ParsedFunction
    expression = '2.9+1.09*x+2.06*y'
  []
  [water_relative_permeability_exact]
    type = ParsedFunction
    expression = '0.28+0.02*x+0.016*y'
  []
  ...
  [gas_relative_permeability_exact]
    type = ParsedFunction
    expression = '0.34+0.006*x+0.004*y'
  []
  ...
  [water_reference_component_storage_exact]
    type = ParsedFunction
    expression = '1.32*0.25*2*(0.2+0.05*x+0.04*y)'
  []
  [oil_reference_component_storage_exact]
    type = ParsedFunction
    expression = '1.32*0.25*3*(0.6-0.08*x-0.06*y)'
  []
  [gas_reference_component_storage_exact]
    type = ParsedFunction
    expression = '1.32*0.25*(0.2+0.03*x+0.02*y)'
  []

  # With F=diag(1.1,1.2), J F^-1 F^-T=diag(1.2/1.1,1.1/1.2).
  [water_reference_component_flux_x_exact]
    type = ParsedFunction
    expression = '-2*0.1*0.9*(1.2/1.1)*(0.28+0.02*x+0.016*y)'
  []
  [water_reference_component_flux_y_exact]
    type = ParsedFunction
    expression = '-2*0.1*1.92*(1.1/1.2)*(0.28+0.02*x+0.016*y)'
  []
  [oil_reference_component_flux_x_exact]
    type = ParsedFunction
    expression = '-3*0.1*1.0*(1.2/1.1)*0.5'
  []
  [oil_reference_component_flux_y_exact]
    type = ParsedFunction
    expression = '-3*0.1*2.0*(1.1/1.2)*0.5'
  []
  [gas_reference_component_flux_x_exact]
    type = ParsedFunction
    expression = '-0.1*1.09*(1.2/1.1)*(0.34+0.006*x+0.004*y)'
  []
  [gas_reference_component_flux_y_exact]
    type = ParsedFunction
    expression = '-0.1*2.06*(1.1/1.2)*(0.34+0.006*x+0.004*y)'
  []

  # The registered material multiplies current sources by J=1.32.
  [water_current_component_source]
    type = ParsedFunction
    expression = '(-2*0.1*0.9*(1.2/1.1)*0.02-2*0.1*1.92*(1.1/1.2)*0.016)/1.32'
  []
  ...
  [gas_current_component_source]
    type = ParsedFunction
    expression = '(-0.1*1.09*(1.2/1.1)*0.006-0.1*2.06*(1.1/1.2)*0.004)/1.32'
  []
  [water_reference_component_source_exact]
    type = ParsedFunction
    expression = '-2*0.1*0.9*(1.2/1.1)*0.02-2*0.1*1.92*(1.1/1.2)*0.016'
  []
  ...
  [gas_reference_component_source_exact]
    type = ParsedFunction
    expression = '-0.1*1.09*(1.2/1.1)*0.006-0.1*2.06*(1.1/1.2)*0.004'
  []
[]
...
[Postprocessors]
  ...
  [water_net_outward_reference_component_flux]
    ...
    pp_names = 'water_right_reference_component_flux water_left_reference_component_flux water_top_reference_component_flux water_bottom_reference_component_flux'
    pp_coefs = '1 -1 1 -1'
  []
  [oil_net_outward_reference_component_flux]
    type = LinearCombinationPostprocessor
    pp_names = 'oil_right_reference_component_flux oil_left_reference_component_flux oil_top_reference_component_flux oil_bottom_reference_component_flux'
    pp_coefs = '1 -1 1 -1'
  []
  [gas_net_outward_reference_component_flux]
    type = LinearCombinationPostprocessor
    pp_names = 'gas_right_reference_component_flux gas_left_reference_component_flux gas_top_reference_component_flux gas_bottom_reference_component_flux'
    pp_coefs = '1 -1 1 -1'
  []
