...
[Materials]
  ...
  [production_well_and_summed_eq32_sources]
    ...
    # The well occupies the complete 2 x 3 x 4 reference domain. Dividing by
    # this analytic completion volume converts total well rates to reference sources.
    completion_reference_volume = 24
    water_surface_density = 2
    oil_surface_density = 3
    gas_surface_density = 1
  []
...
[Postprocessors]
  ...
  []
  # On a single TET10 macrocell, a quadrature-weighted squared roundoff error
  # can be slightly negative. Verify this spatially constant rate through its
  # AD material average and an explicit exact-value difference instead.
  [stock_tank_gas_component_rate_average]
    type = ADElementAverageMaterialProperty
    mat_prop = black_oil_gas_reference_component_storage_rate
  []
  [stock_tank_gas_component_rate_exact]
    type = ConstantPostprocessor
    value = -0.0004027777777777778
  []
  [sum_xi_J_phi_xi_fluid_intrinsic_density_xi_eta_xi_stock_tank_gas_component_rate_l2]
    type = LinearCombinationPostprocessor
    pp_names = 'stock_tank_gas_component_rate_average stock_tank_gas_component_rate_exact'
    pp_coefs = '1 -1'
  []
...
[Executioner]
  ...
  solve_type = NEWTON
  start_time = 0
  # J/dt = 1.716/37.44 = 1.1/24, so the manufactured state jump produces
  # storage rates per reference volume equal to the well totals divided by 24.
  dt = 37.44
  num_steps = 1
  nl_abs_tol = 1e-13
  nl_rel_tol = 1e-13
