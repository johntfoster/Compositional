...
[Functions]
  ...
  [conductivity_exact]
    type = ParsedFunction
    expression = '1.331*(3.5+x+2*y+3*z)'
  []
  [current_heat_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-0.605*(3.5+x+2*y+3*z)^2'
  []
  [current_heat_flux_x_exact]
    type = ParsedFunction
    expression = '-1.21*(3.5+x+2*y+3*z)'
  []
  [current_heat_flux_y_exact]
    type = ParsedFunction
    expression = '-2.42*(3.5+x+2*y+3*z)'
  []
  [current_heat_flux_z_exact]
    type = ParsedFunction
    expression = '-3.63*(3.5+x+2*y+3*z)'
  []
  [reference_heat_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-0.73205*(3.5+x+2*y+3*z)^2'
  []
  [reference_heat_flux_x_exact]
    type = ParsedFunction
    expression = '-1.4641*(3.5+x+2*y+3*z)'
  []
  [reference_heat_flux_y_exact]
    type = ParsedFunction
    expression = '-2.9282*(3.5+x+2*y+3*z)'
  []
  [reference_heat_flux_z_exact]
    type = ParsedFunction
    expression = '-4.3923*(3.5+x+2*y+3*z)'
  []
...
[AuxKernels]
  ...
  [current_heat_flux_y_output]
    ...
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [current_heat_flux_z_output]
    type = ADMaterialRealVectorValueAux
    variable = current_heat_flux_z_aux
    property = current_heat_flux
    component = 2
    execute_on = 'INITIAL TIMESTEP_END'
  []
  ...
  [reference_heat_flux_y_output]
    ...
    execute_on = 'INITIAL TIMESTEP_END'
  []
  [reference_heat_flux_z_output]
    type = ADMaterialRealVectorValueAux
    variable = reference_heat_flux_z_aux
    property = reference_heat_flux
    component = 2
    execute_on = 'INITIAL TIMESTEP_END'
  []
...
[Postprocessors]
  ...
  [current_heat_flux_z_l2]
    type = ElementL2Error
    variable = current_heat_flux_z_aux
    function = current_heat_flux_z_exact
  []
  ...
  [reference_heat_flux_z_l2]
    type = ElementL2Error
    variable = reference_heat_flux_z_aux
    function = reference_heat_flux_z_exact
  []
  ...
  [net_outward_reference_heat_flux]
    ...
    pp_names = 'right_reference_heat_flux_x left_reference_heat_flux_x top_reference_heat_flux_y bottom_reference_heat_flux_y front_reference_heat_flux_z back_reference_heat_flux_z'
