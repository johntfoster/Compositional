...
[Functions]
  [ux_exact]
    type = ParsedFunction
    expression = '(${stretch_x}-1)*x'
  []
  [uy_exact]
    type = ParsedFunction
    expression = '(${stretch_y}-1)*y'
  []
  [uz_exact]
    type = ParsedFunction
    expression = '(${stretch_z}-1)*z'
  []
[]
...
[Materials]
  [double_prime_stress]
    ...
    shear_modulus = 2
    skeleton_bulk_modulus = 4
    mineral_bulk_modulus = 10
    reference_solid_volume_fraction = 0.7
  []
  [biot_transform]
