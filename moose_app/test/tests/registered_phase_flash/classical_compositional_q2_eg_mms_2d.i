...
[Functions]
  ...
  [phase_intrinsic_density_exact]
    ...
    expression = '3'
  []
  # Let s=0.72*x+0.605*y.  Since each production Darcy phase flux is
  # W=(-0.72,-0.605)=-Grad(s), every phase/component flux below has an
  # independent analytic potential while retaining nonzero x and y transport.
  [eta_oil_0_exact]
    type = ParsedFunction
    expression = '0.7+0.1*(0.72*x+0.605*y)'
  []
  [eta_oil_1_exact]
    type = ParsedFunction
    expression = '0.3-0.1*(0.72*x+0.605*y)'
  []
  [eta_gas_0_exact]
    type = ParsedFunction
    expression = '0.2+0.05*(0.72*x+0.605*y)'
  []
  [eta_gas_1_exact]
    type = ParsedFunction
    expression = '0.8-0.05*(0.72*x+0.605*y)'
  []
  [z0_exact]
    type = ParsedFunction
    expression = '8/15+(0.72*x+0.605*y)/12'
  []
  [z1_exact]
    type = ParsedFunction
    expression = '7/15-(0.72*x+0.605*y)/12'
  []
  ...
  [direct_summed_eq32_component0_reference_storage_exact]
    type = ParsedFunction
    expression = '1.32*(0.2*3*(0.7+0.1*(0.72*x+0.605*y))+0.1*3*(0.2+0.05*(0.72*x+0.605*y)))'
  []
  [direct_summed_eq32_component1_reference_storage_exact]
    type = ParsedFunction
    expression = '1.32*(0.2*3*(0.3-0.1*(0.72*x+0.605*y))+0.1*3*(0.8-0.05*(0.72*x+0.605*y)))'
  []
  [oil_reference_relative_mass_flux_potential]
    type = ParsedFunction
    expression = '-0.72*x-0.605*y'
  []
  ...
  [oil_component0_reference_flux_potential]
    type = ParsedFunction
    expression = '-0.7*(0.72*x+0.605*y)-0.05*(0.72*x+0.605*y)^2'
  []
  [oil_component1_reference_flux_potential]
    type = ParsedFunction
    expression = '-0.3*(0.72*x+0.605*y)+0.05*(0.72*x+0.605*y)^2'
  []
  [gas_component0_reference_flux_potential]
    type = ParsedFunction
    expression = '-0.2*(0.72*x+0.605*y)-0.025*(0.72*x+0.605*y)^2'
  []
  [gas_component1_reference_flux_potential]
    type = ParsedFunction
    expression = '-0.8*(0.72*x+0.605*y)+0.025*(0.72*x+0.605*y)^2'
  []
  [component0_reference_flux_potential]
    type = ParsedFunction
    expression = '-0.9*(0.72*x+0.605*y)-0.075*(0.72*x+0.605*y)^2'
  []
  [component1_reference_flux_potential]
    type = ParsedFunction
    expression = '-1.1*(0.72*x+0.605*y)+0.075*(0.72*x+0.605*y)^2'
  []
  [component0_current_source_exact]
    type = ParsedFunction
    expression = '-0.13266375/1.32'
  []
...
[Postprocessors]
  ...
  [component0_net_outward_reference_flux]
    ...
    pp_names = 'component0_right_reference_flux component0_left_reference_flux component0_top_reference_flux component0_bottom_reference_flux'
    pp_coefs = '1 -1 1 -1'
  []
  [component1_net_outward_reference_flux]
    type = LinearCombinationPostprocessor
    pp_names = 'component1_right_reference_flux component1_left_reference_flux component1_top_reference_flux component1_bottom_reference_flux'
    pp_coefs = '1 -1 1 -1'
  []
