...
[Functions]
  ...
  [water_phi_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,0,0.04)'
  []
  [water_active_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,0,1)'
  []
  [phi_total_exact]
    type = ParsedFunction
    expression = '0.3 + if(z<0.5,0,0.04)'
  []
  [phi_oil_exact]
    type = ParsedFunction
    expression = '0.2'
  ...
  [rho_water_0_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,2.5,1)'
  []
  [rho_water_1_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,0.5,2)'
  []
  [eta0_exact]
    type = ParsedFunction
    expression = '1/3'
  ...
  [eta_water_0_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,0.8,1/3)'
  []
  [eta_water_1_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,0.05,2/3)'
  []
  [storage0_exact]
    type = ParsedFunction
    expression = '0.3 + if(z<0.5,0,0.04)'
  ...
  [storage1_exact]
    type = ParsedFunction
    expression = '0.6 + 2*if(z<0.5,0,0.04)'
  []
  [oil_saturation_exact]
    type = ParsedFunction
    expression = '0.2/(0.3 + if(z<0.5,0,0.04))'
  []
  [gas_saturation_exact]
    type = ParsedFunction
    expression = '0.1/(0.3 + if(z<0.5,0,0.04))'
  []
  [water_saturation_exact]
    type = ParsedFunction
    expression = 'if(z<0.5,0,0.04)/(0.3 + if(z<0.5,0,0.04))'
  []
[]
...
[Materials]
  ...
  [water_helmholtz]
    type = ADDerivativeParsedMaterial
    coupled_variables = 'rho_water_0 rho_water_1 temperature'
    property_name = water_helmholtz_density
    constant_names = 'K rho_ref R'
    constant_expressions = '4 2 5'
    expression = '0.5*K*(rho_water_0+rho_water_1-rho_ref)^2 + R*temperature*(rho_water_0*(log(rho_water_0)-1)+rho_water_1*(log(rho_water_1)-1))'
    derivative_order = 2
    enable_jit = true
  []
  [oil_eos]
  ...
  [water_eos]
    ...
    phase = water
    phase_registry = phases
    partial_densities = 'rho_water_0 rho_water_1'
    temperature = temperature
    porosity = phi_water
    helmholtz_density_name = water_helmholtz_density
  []
  [flash]
...
[AuxKernels]
  ...
  [gas_saturation_aux]
    ...
    execute_on = INITIAL
  []
  [water_saturation_aux]
    type = ADMaterialRealAux
    variable = water_saturation
    property = registered_flash_water_saturation
    execute_on = INITIAL
  []
  [water_active_aux]
    type = ADMaterialRealAux
  ...
  [volume_residual_aux]
    ...
    execute_on = INITIAL
  []
  [water_fraction_residual_aux]
    type = ADMaterialRealAux
    variable = water_fraction_residual
    property = registered_flash_water_component_mass_fraction_residual
    execute_on = INITIAL
  []
  [water_mu0_residual_aux]
    ...
    execute_on = INITIAL
  []
  [water_mu1_residual_aux]
    type = ADMaterialRealAux
    variable = water_mu1_residual
    property = registered_flash_water_chemical_equilibrium_residual_1
    execute_on = INITIAL
  []
  [water_pressure_residual_aux]
    type = ADMaterialRealAux
    variable = water_pressure_residual
    property = registered_flash_water_pressure_equilibrium_residual
    execute_on = INITIAL
  []
...
[Postprocessors]
  ...
  [water_mu0_residual_l2]
    ...
    execute_on = INITIAL
  []
  [water_mu1_residual_l2]
    type = ElementL2Error
    variable = water_mu1_residual
    function = zero
    execute_on = INITIAL
  []
  [water_pressure_residual_l2]
    type = ElementL2Error
    variable = water_pressure_residual
    function = zero
    execute_on = INITIAL
