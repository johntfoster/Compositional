...
[Functions]
  ...
  [a_exact]
    type = ParsedFunction
    expression = '(1+0.1*t)^2*(1-0.05*t)*(2+0.1*t)/2'
  []
  [storage_exact]
    type = ParsedFunction
  ...
  [solid_fraction_exact]
    type = ParsedFunction
    expression = '(0.7+0.03*t)/((1+0.1*t)^2*(1-0.05*t)*(2+0.1*t))'
  []
  [source_exact]
    type = ParsedFunction
    expression = '0.03/((1+0.1*t)^2*(1-0.05*t))'
  []
  [distension_time_forcing]
    type = ParsedFunction
    expression = '(((1+0.1*t)^2*(1-0.05*t)*(2+0.1*t)-(1+0.1*(t-0.1))^2*(1-0.05*(t-0.1))*(2+0.1*(t-0.1)))/(0.1*(1+0.1*t)^2*(1-0.05*t)*(2+0.1*t)))-(2*((1+0.1*t)-(1+0.1*(t-0.1)))/(0.1*(1+0.1*t))+((1-0.05*t)-(1-0.05*(t-0.1)))/(0.1*(1-0.05*t)))-((2+0.1*t)-(2+0.1*(t-0.1)))/(0.1*(2+0.1*t))'
  []
  [zero]
