...
[Functions]
  ...
  [rho0_exact]
    type = ParsedFunction
    expression = '1+0.1*x+0.2*y+0.05*t'
  []
  [rho1_exact]
    type = ParsedFunction
    expression = '2*(1+0.1*x+0.2*y+0.05*t)'
  []
  [temperature_backbone_exact]
    type = ParsedFunction
    expression = 'x+(288/121)*y+0.2*t'
  []
  ...
  [temperature_exact]
    type = ParsedFunction
    expression = '1+x+(288/121)*y+0.2*t'
  []
  ...
  [pressure_backbone_exact]
    type = ParsedFunction
    expression = '2*x+(576/121)*y+0.4*t'
  []
  ...
  [pressure_exact]
    type = ParsedFunction
    expression = '9+2*x+(576/121)*y+0.4*t'
  []
  [neutral_potential_0_backbone_exact]
    type = ParsedFunction
    expression = '3*x+(864/121)*y+0.6*t'
  []
  ...
  [neutral_potential_0_exact]
    type = ParsedFunction
    expression = '5+3*x+(864/121)*y+0.6*t'
  []
  [neutral_potential_1_backbone_exact]
    type = ParsedFunction
    expression = '4*x+(1152/121)*y+0.8*t'
  []
  ...
  [neutral_potential_1_exact]
    type = ParsedFunction
    expression = '9+4*x+(1152/121)*y+0.8*t'
  []
  [helmholtz_exact]
    type = ParsedFunction
    expression = '(1+0.1*x+0.2*y+0.05*t)*(12+11*(1+x+(288/121)*y+0.2*t))-7-2*(1+x+(288/121)*y+0.2*t)'
  []
  [intrinsic_density_exact]
    type = ParsedFunction
    expression = '3*(1+0.1*x+0.2*y+0.05*t)'
  []
  [bulk_phase_density_exact]
    type = ParsedFunction
    expression = '0.75*(1+0.1*x+0.2*y+0.05*t)'
  []
  [specific_helmholtz_exact]
    type = ParsedFunction
    expression = '((1+0.1*x+0.2*y+0.05*t)*(12+11*(1+x+(288/121)*y+0.2*t))-7-2*(1+x+(288/121)*y+0.2*t))/(3*(1+0.1*x+0.2*y+0.05*t))'
  []
  [entropy_density_exact]
    type = ParsedFunction
    expression = '2-11*(1+0.1*x+0.2*y+0.05*t)'
  []
  [summed_reference_component_0_exact]
    type = ParsedFunction
    expression = '1.32*0.25*(1+0.1*x+0.2*y+0.05*t)'
  []
  [summed_reference_component_1_exact]
    type = ParsedFunction
    expression = '1.32*0.25*2*(1+0.1*x+0.2*y+0.05*t)'
  []
  ...
  [current_component_0_source]
    type = ParsedFunction
    expression = '-0.2235/1.32'
  []
  [current_component_1_source]
    type = ParsedFunction
    expression = '-0.447/1.32'
  []
  ...
  [oil_reference_relative_mass_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-1.44*((1+0.05*t)*(x+2*y)+0.05*(x+2*y)^2)'
  []
  [current_component_0_extra_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-0.3*x-(864/1210)*y'
  []
  [current_component_1_extra_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-0.4*x-(1152/1210)*y'
  []
  [reference_component_0_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-0.48*((1+0.05*t)*(x+2*y)+0.05*(x+2*y)^2)-0.36*x-(2376/3025)*y'
  []
  [reference_component_1_flux_antiderivative_exact]
    type = ParsedFunction
    expression = '-0.96*((1+0.05*t)*(x+2*y)+0.05*(x+2*y)^2)-0.48*x-(3168/3025)*y'
  []
  [zero]
    type = ParsedFunction
    expression = '0'
...
[Postprocessors]
  ...
  [right_reference_component_0_flux]
    ...
    component = 0
  []
  [bottom_reference_component_0_flux]
    type = ADSideIntegralMaterialProperty
    boundary = bottom
    property = reference_component_0_flux
    component = 1
  []
  [top_reference_component_0_flux]
    type = ADSideIntegralMaterialProperty
    boundary = top
    property = reference_component_0_flux
    component = 1
  ...
  [right_reference_component_1_flux]
    ...
    component = 0
  []
  [bottom_reference_component_1_flux]
    type = ADSideIntegralMaterialProperty
    boundary = bottom
    property = reference_component_1_flux
    component = 1
  []
  [top_reference_component_1_flux]
    type = ADSideIntegralMaterialProperty
    boundary = top
    property = reference_component_1_flux
    component = 1
  ...
  [net_outward_reference_component_0_flux]
    ...
    pp_names = 'right_reference_component_0_flux left_reference_component_0_flux top_reference_component_0_flux bottom_reference_component_0_flux'
    pp_coefs = '1 -1 1 -1'
  []
  [net_outward_reference_component_1_flux]
    type = LinearCombinationPostprocessor
    pp_names = 'right_reference_component_1_flux left_reference_component_1_flux top_reference_component_1_flux bottom_reference_component_1_flux'
    pp_coefs = '1 -1 1 -1'
  []
