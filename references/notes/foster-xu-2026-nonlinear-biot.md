# Foster and Xu 2026, Revisiting finite deformation poromechanics

## Source

- PDF: `references/pdfs/foster-xu-2026-nonlinear-biot.pdf`
- BibTeX key:
- Authors/year: John T. Foster and Xiao Xu, 2026
- DOI or URL:

## Why It Matters

- Derives a nonlinear Biot coefficient from a finite-deformation mixture-theory formulation.
- Uses an extended Hamilton principle for a binary solid-fluid mixture.
- Uses a Legendre transformation to express the solid energetic contribution in terms of pressure rather than specific volume.
- Provides a direct comparison point for the current manuscript's finite-deformation poromechanics and Biot-coefficient mixture rules.

## Definitions and Assumptions

- The manuscript focuses on a binary solid-fluid mixture.
- The nonlinear Biot coefficient is interpreted through the local solid specific-volume response to skeleton deformation at fixed pore pressure.
- The small-deformation limit is compared against the classical Biot coefficient.

## Equations or Results to Compare

- Introduction: nonlinear Biot coefficient stated in terms of the derivative of current local solid specific volume with respect to the solid skeleton Jacobian at fixed pore pressure.
- Variational derivation: momentum equations are developed from an extended Hamilton principle following Bedford and Drumheller.
- Small-deformation comparison: checks reduction to the classical Biot-type coefficient.

## Useful Quotes or Page Anchors

- Page 1: title, authors, date, and abstract.
- Page 2: motivation for deriving the coefficient without constitutive assumptions on its form.

## Manuscript Connections

- Related TeX locations: `sections/variational_derivation.tex`, `sections/coleman_noll.tex`
- Possible citation location: nonlinear Biot coefficient discussion, finite-deformation poromechanics comparison, or correspondence to other theories.

## Open Questions

- Confirm the preferred citation key and whether this should be cited as an unpublished manuscript, preprint, or submitted article.
