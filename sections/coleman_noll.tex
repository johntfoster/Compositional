...
We write
\begin{equation}
	\left.\p{\bar{\mbf F}_s^\theta}{\eta_s^\beta}\heldfixed{\mc S}{\eta_s^\beta}
	=\sum_\gamma
	\left.\p{\bar{\mbf F}_s^\theta}{\phi_s^\gamma}\heldfixed{\mc T_s}{\phi_s^\gamma}
	\left.\p{\phi_s^\gamma}{\eta_s^\beta}\heldfixed{\mc S}{\eta_s^\beta} .
	\label{eq:CN_Ftheta_eta_chain}
\end{equation}
For the mobile fluid phases, we postulate the Coleman--Noll state variables
\begin{equation}
	\mc F
	=\left\{\rho_\ell,\eta_\ell^\beta,\theta,\mc A_m\right\},
	\qquad
...
\begin{align}
	...
	\left.\p{\psi_\ell^\alpha}{\rho_\ell}\heldfixed{\mc F}{\rho_\ell}\dot\rho_\ell
	+\sum_\beta\left.\p{\psi_\ell^\alpha}{\eta_\ell^\beta}\heldfixed{\mc F}{\eta_\ell^\beta}\dot\eta_\ell^\beta
	+\left.\p{\psi_\ell^\alpha}{\theta}\heldfixed{\mc F}{\theta}\dot\theta
	+\left.\p{\psi_\ell^\alpha}{\mc A_m}\heldfixed{\mc F}{\mc A_m}\dot{\mc A}_m
	\right).
...
\begin{align}
	...
	\left.\p{\psi_s^\alpha}{\bar{\mbf{F}}^M_s}\heldfixed{\mc S}{\bar{\mbf F}_s^M}
	:\dot{\bar{\mbf{F}}}^M_s
	+\left.\p{\psi_s^\alpha}{a_s}\heldfixed{\mc S}{a_s}\dot{a}_s
	\notag                                                     \\
	 & \qquad
	+\left.\p{\psi_s^\alpha}{\bar\rho_s}\heldfixed{\mc S}{\bar\rho_s}\dot{\bar\rho}_s
	+\sum_\beta\left.\p{\psi_s^\alpha}{\eta_s^\beta}\heldfixed{\mc S}{\eta_s^\beta}\dot\eta_s^\beta
	\notag                                                     \\
	 & \qquad
	+\left.\p{\psi_s^\alpha}{\mc{A}_m}\heldfixed{\mc S}{\mc A_m}\dot{\mc{A}}_m
	+\left.\p{\psi_s^\alpha}{\theta}\heldfixed{\mc S}{\theta}\dot\theta
	\bigg).
...
\begin{align}
	...
	-\bar{\mbf F}_s^M
	\left.\frac{\partial\bar{\mbf F}_s^\theta}{\partial\theta}\heldfixed{\mc T_s}{\theta}
...
\begin{equation}
	\mbf H_s^\alpha
	= (\bar{\mbf F}_s^M)^T
	\left.\p{\psi_s^\alpha}{\bar{\mbf F}_s^M}\heldfixed{\mc S}{\bar{\mbf F}_s^M}
	(\bar{\mbf F}_s^\theta)^{-T} .
	\label{eq:CN_H_def}
\end{equation}
The mobile-phase Helmholtz energies are phase EOS potentials using the
fluid-state variables in \eqref{eq:CN_fluid_state_variables}.
\begin{align}
...
\begin{align}
	...
	\notag                                               \\
	       & +\sum_\ell\bs\sigma_\ell':\mbf L_\ell
	\notag                                               \\
	       & +\left[
		\frac{1}{3a_s}\operatorname{tr}(\bs\sigma_s')
		-\rho_s\sum_\alpha\eta_s^\alpha
		\left.\p{\psi_s^\alpha}{a_s}\heldfixed{\mc S}{a_s}
		\right]\dot a_s
	\notag                                               \\
	       & +\sum_\xi \left[
		\frac{\phi_\xi \bar p_\xi}{\bar\rho_\xi}
		-\rho_\xi\sum_\alpha\eta_\xi^\alpha
		\left.\p{\psi_\xi^\alpha}{\bar\rho_\xi}\heldfixed{\mc F}{\bar\rho_\xi}
		\right]\dot{\bar\rho}_\xi
	-\sum_\ell\rho_\ell\sum_\alpha\eta_\ell^\alpha
	\left.\p{\psi_\ell^\alpha}{\rho_\ell}\heldfixed{\mc F}{\rho_\ell}\dot\rho_\ell
	\notag                                               \\
	       & -\sum_\xi\rho_\xi\sum_\alpha\eta_\xi^\alpha
	\left[
		s_\xi^\alpha+\left.\p{\psi_\xi^\alpha}{\theta}\heldfixed{\mc F}{\theta}
		\right]\dot\theta
	+\rho_s\sum_\alpha\eta_s^\alpha
	\mbf H_s^\alpha:
	\left.\p{\bar{\mbf F}_s^\theta}{\theta}\heldfixed{\mc T_s}{\theta}\dot\theta
	\notag                                               \\
	       & -\sum_\xi\rho_\xi\sum_\beta\left[
		\sum_\alpha\eta_\xi^\alpha\left(
		\left.\p{\psi_\xi^\alpha}{\eta_\xi^\beta}\heldfixed{\mc F}{\eta_\xi^\beta}
		\right)\right]\dot\eta_\xi^\beta
	...
	\notag                                               \\
	       & -\sum_\xi\rho_\xi\sum_\alpha\eta_\xi^\alpha
	\left.\p{\psi_\xi^\alpha}{\mc A_m}\heldfixed{\mc F}{\mc A_m}\dot{\mc A}_m
	-\sum_m\left(
...
\begin{align}
	...
	\left.\p{\psi_s^\alpha}{\bar\rho_s}\heldfixed{\mc S}{\bar\rho_s} .
...
\begin{equation}
	...
	\left.\p{\psi_s^\alpha}{\theta}\heldfixed{\mc S}{\theta}
	+\sum_\alpha\eta_s^\alpha
	\mbf H_s^\alpha:
	\left.\p{\bar{\mbf F}_s^\theta}{\theta}\heldfixed{\mc T_s}{\theta} .
...
composition forces are fixed.  The stress-free compositional dilation modifies
the usual composition coefficient by
\begin{equation}
	-\rho_s\sum_\alpha\eta_s^\alpha\left(
	\left.\p{\psi_s^\alpha}{\eta_s^\beta}\heldfixed{\mc S}{\eta_s^\beta}
	-\mbf H_s^\alpha:
	\left.\p{\bar{\mbf F}_s^\theta}{\eta_s^\beta}\heldfixed{\mc S}{\eta_s^\beta}
	\right).
...
\begin{align}
	...
	\notag                                   \\
	       & -\rho_s\sum_\alpha\eta_s^\alpha
	\left.\p{\psi_s^\alpha}{\mc A_m}\heldfixed{\mc S}{\mc A_m}\dot{\mc A}_m
	+\mc D_{\rm inelastic} .
