% !TeX root = ../main.tex
\section{Conservation of charge}
\label{sec:conservation_of_charge}

Electric charge is carried by the same phase/component material measures as
mass.  Weighting each component balance by the charge carried per unit mass and
summing gives the charge balance.  The field equation for the
electric potential and the electrochemical component potential are introduced
later in \sref{virtual_power_derivation}.

Let \(z_\xi^\alpha\) denote the charge carried per unit mass of component
\(\alpha\) in phase \(\xi\).  The charge density per unit current mixture volume
associated with that component is
%
\begin{equation}
	\varrho_\xi^\alpha
	=
	z_\xi^\alpha\rho_\xi^\alpha
	=
	z_\xi^\alpha\phi_\xi\bar\rho_\xi\eta_\xi^\alpha .
	\label{eq:component_charge_density_definition}
\end{equation}
%
The coefficient \(z_\xi^\alpha\) may vanish for neutral components, may be a
constant valence-to-mass ratio for a mobile ion, or may be an effective charge
per mass for a solid component representing fixed surface or exchange-site
charge.  Fixed clay-surface or exchange-site charge and mobile aqueous charge
are represented by the same phase/component variables
\cite{leroy2004triple,newman2004electrochemical}.  Let
\(\varrho_{\rm ext}\) denote any independently prescribed external bulk-charge
density.  The total free charge density is
%
\begin{equation}
	\varrho
	=
	\sum_\xi\sum_\alpha\varrho_\xi^\alpha
	+
	\varrho_{\rm ext}
	=
	\sum_\xi\sum_\alpha
	z_\xi^\alpha\phi_\xi\bar\rho_\xi\eta_\xi^\alpha
	+
	\varrho_{\rm ext}.
	\label{eq:mixture_charge_density_definition}
\end{equation}
%
The external contribution is completed by a prescribed spatial current
\(\mbf i_{\rm ext}\) and charge-supply rate \(s_{\rm ext}\) satisfying
%
\begin{equation}
	\frac{\partial\varrho_{\rm ext}}{\partial t}
	+
	\nabla_{\mbf x}\cdot\mbf i_{\rm ext}
	=
	s_{\rm ext}.
	\label{eq:external_charge_loading_balance}
\end{equation}
%
Only two members of
\(\{\varrho_{\rm ext},\mbf i_{\rm ext},s_{\rm ext}\}\) may be specified
independently.  A static spatial loading has
\(\partial\varrho_{\rm ext}/\partial t=0\),
\(\mbf i_{\rm ext}=\mbf0\), and \(s_{\rm ext}=0\).  A transported external
charge may instead use
\(\mbf i_{\rm ext}=\varrho_{\rm ext}\mbf u_{\rm ext}\), with
\eqref{eq:external_charge_loading_balance} determining its supply.  A
time-dependent supplied loading permits nonzero \(s_{\rm ext}\), again subject
to that balance.  The total \(\varrho\) is the field-source coefficient in the
electrical virtual work, Gauss' law, and the bulk field-source power
\(-\varrho\dot\varphi\).  The prescribed triplet is not included in the
component balances below and is not varied with the material coordinates.
Representing fixed charge as a solid component instead ties charge storage,
deformation, reaction, and ion exchange to the same material measures used for
mass; such material charge must not also be included in
\(\varrho_{\rm ext}\).

The material charge measure follows by multiplying the augmented component
material measure \eqref{eq:augmented_component_material_measure} by
\(z_\xi^\alpha\),
%
\begin{equation}
	z_\xi^\alpha\rho_{\xi0}^\alpha
	+
	z_\xi^\alpha\mc C_\xi^\alpha
	=
	J_\xi\varrho_\xi^\alpha .
	\label{eq:augmented_component_material_charge_measure}
\end{equation}
%
For constant \(z_\xi^\alpha\), its material rate gives
%
\begin{align}
	\frac{
	z_\xi^\alpha\phasedot{\mc C}_\xi^\alpha
	}{J_\xi}
	&=
	\frac{D_\xi\varrho_\xi^\alpha}{Dt}
	+
	\varrho_\xi^\alpha\nabla_{\mbf x}\cdot\mbf v_\xi .
	\label{eq:component_charge_material_rate}
\end{align}
%
If an effective charge \(z_\xi^\alpha\) is taken to depend on state, then the
left-hand side acquires the corresponding material derivative of
\(z_\xi^\alpha\).  The fixed-\(z_\xi^\alpha\) case is the charge-weighted
component mass balance used below.

Substitution of the rate \(\phasedot{\mc C}_\xi^\alpha\) from
\eqref{eq:reference_conversion_accumulation_rate}
gives the charge-weighted current-volume identity
%
\begin{align}
	\frac{
	z_\xi^\alpha\phasedot{\mc C}_\xi^\alpha
	}{J_\xi}
	-
	z_\xi^\alpha
	\sum_m\nu_{\xi (m)}^\alpha\dot r_{(m)}
	+
	z_\xi^\alpha
	\nabla_{\mbf x}\cdot
	\left(
	\mbf j_{\xi,{\rm disp}}^\alpha
	+
	\mbf j_{\xi,{\rm diff}}^\alpha
	\right)
	=
	0 .
	\label{eq:component_charge_rate_identity}
\end{align}
%
This identity is the component material-rate balance viewed through the charge
weights \(z_\xi^\alpha\).  Its spatial form is the charge balance for each
charged component,
%
\begin{align}
	\frac{\partial\varrho_\xi^\alpha}{\partial t}
	+
	\nabla_{\mbf x}\cdot
	\left[
	z_\xi^\alpha
	\left(
	\rho_\xi^\alpha\mbf v_\xi
	+
	\mbf j_{\xi,{\rm disp}}^\alpha
	+
	\mbf j_{\xi,{\rm diff}}^\alpha
	\right)
	\right]
	&=
	z_\xi^\alpha
	\sum_m\nu_{\xi (m)}^\alpha\dot r_{(m)} .
	\label{eq:component_charge_balance}
\end{align}
%
The current density carried by phase motion and relative component transport is
%
\begin{equation}
	\mbf i
	=
	\sum_\xi\sum_\alpha
	z_\xi^\alpha
	\left(
	\rho_\xi^\alpha\mbf v_\xi
	+
	\mbf j_{\xi,{\rm disp}}^\alpha
	+
	\mbf j_{\xi,{\rm diff}}^\alpha
	\right).
	\label{eq:charge_current_density_definition}
\end{equation}
%
Summing \eqref{eq:component_charge_balance} over phases and components gives
%
\begin{align}
	\frac{\partial}{\partial t}
	\left(
	\sum_\xi\sum_\alpha\varrho_\xi^\alpha
	\right)
	+
	\nabla_{\mbf x}\cdot\mbf i
	=
	\sum_m
	\left(
	\sum_\xi\sum_\alpha
	z_\xi^\alpha\nu_{\xi (m)}^\alpha
	\right)
	\dot r_{(m)} .
	\label{eq:mixture_charge_balance_with_sources}
\end{align}
%
Local internal mechanisms must conserve electric charge,
%
\begin{equation}
	\sum_\xi\sum_\alpha
	z_\xi^\alpha\nu_{\xi (m)}^\alpha
	=0,
	\qquad
	\text{for each mechanism }(m),
	\label{eq:mechanism_charge_conservation}
\end{equation}
%
so that the material-carried charge satisfies
%
\begin{equation}
	\frac{\partial}{\partial t}
	\left(
	\sum_\xi\sum_\alpha\varrho_\xi^\alpha
	\right)
	+
	\nabla_{\mbf x}\cdot\mbf i
	=0 .
	\label{eq:mixture_material_charge_balance}
\end{equation}
%
Combining this result with
\eqref{eq:external_charge_loading_balance} gives the total charge balance
%
\begin{equation}
	\frac{\partial\varrho}{\partial t}
	+
	\nabla_{\mbf x}\cdot
	\left(
	\mbf i+\mbf i_{\rm ext}
	\right)
	=
	s_{\rm ext}.
	\label{eq:mixture_charge_balance}
\end{equation}
%
Using the skeleton-following rate convention
\eqref{eq:MC_skeleton_rate_convention}, the same balance is
%
\begin{equation}
	\dot\varrho
	+
	\varrho
	\nabla_{\mbf x}\cdot\mbf v_{\mc S}
	+
	\nabla_{\mbf x}\cdot
	\left(
	\mbf i+\mbf i_{\rm ext}
	-\varrho\mbf v_{\mc S}
	\right)
	=
	s_{\rm ext}.
	\label{eq:mixture_charge_balance_skeleton_observer}
\end{equation}
%
Equation \eqref{eq:mechanism_charge_conservation} is the charge analogue of the
mass-conservation restriction
\(\sum_\xi\sum_\alpha\nu_{\xi (m)}^\alpha=0\).  As one example, an ion-exchange
mechanism between an aqueous phase and charged solid sites may remove an aqueous
component and a solid-site component while creating the exchanged aqueous and
solid-site components.  This mechanism changes fluid composition and solid-site
occupancy while satisfying both mass conservation and
\eqref{eq:mechanism_charge_conservation}.  More generally, the phase/component
formulation admits exchange among any represented phases and components whose
stoichiometry satisfies these two conservation restrictions.  Thus charge
attached to deforming solid sites is transported by the component current
\(\mbf i\); the external current and supply describe only the independently
prescribed loading.

Ion-exchange and surface-complexation models are represented by retaining
charged solid-site components, imposing
\eqref{eq:mechanism_charge_conservation} on the exchange stoichiometry, and
choosing admissible kinetic or equilibrium closures for the corresponding
mechanism rates.  The electrostatic field equation is Gauss' law, derived in
\sref{virtual_power_derivation}.
