...
$\mc B$.  Greek phase labels such as $\xi$ and $\zeta$ identify material that moves with the same
...
% AGENT-LOCK-END
%
The associated phase deformation gradient $\mbf{F}_\xi$ maps differential vectors from the phase reference coordinates to the current coordinates, and its determinant \(J_\xi\) is the phase Jacobian:
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{align}
...
\end{align}
% AGENT-LOCK-END
where $\mbf{u}_\xi = \mbf{u}_\xi(\mbf{X}_\xi, t)$ is the phase displacement field, i.e.\ $\mbf{u}_\xi = \mbf{x} - \mbf{X}_\xi$, and $\nabla_{\mbf{X}_\xi}$ is the gradient operator in the phase reference coordinates.
...
conversion includes chemical reactions, sorption/desorption, and pure phase
transformations.  The
...
$\nu_{\xi (m)}^\alpha$, where the parenthesized index \((m)\) labels the conversion mechanism.  Thus
...
mixture volume.  The current production rate for each phase/component pair is
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
...
pure phase transformations.  A pure phase transformation of component $\alpha$ from phase
...
% AGENT-LOCK-END
%
The right-hand side of \eqref{eq:spatial_mass_3} vanishes by mass conservation
for each internal mechanism:
$\sum_\xi\sum_\alpha\nu_{\xi (m)}^\alpha=0$.
...
the phase/component conversion constraint
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{align}
	\frac{\dot{\mc{C}^\alpha_\xi}}{J_\xi}
