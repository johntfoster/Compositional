...
\end{equation}
%
with concrete non-field state sets
%
\begin{equation}
	\mc X_s
	=
...
These terms belong to the reversible part of the entropy accounting.  The
present reservoir-scale model omits independent polarization, dielectric
relaxation, and switching; their induced dielectric effect can still be
represented, when needed, by the constitutive choice of \(\omega^{+}_{\xi}\), or
equivalently by a permittivity closure for \(\mbf d_\xi\).
...
combined reversible stress \(\bs\sigma_\xi^{\prime+}\).  When
\(\omega^{+}_{\xi}\) is retained as an explicit constitutive potential and
depends on a mechanical, compositional, or thermal state variable, its partial
derivative is added to the matching reversible coefficient before imposing the
corresponding Coleman--Noll restriction.
...
The electrical enthalpy rate is expanded only over the appropriate non-field
state set, \(\mc X_f\) or \(\mc X_s\); the electric-field rate has already been
converted by the potential variation into the Gauss-law coefficient above.  Thus
the terms
...
compatibility relation for the transfer potential,
%
\begin{empheq}[box=\fbox]{equation}
	\frac{D_\xi\tau}{Dt}
	=
...
The Bedford--Drumheller transfer potential is the single scalar \(\tau\), so the
right-hand side must have a common value for all components in the same phase.
Equivalently, for any two components
...
well-defined.  One component relation, together with an initial value, evolves
\(\tau\) along the phase motion; the remaining component relations constrain the
storage multipliers.
%
...
\end{equation}
%
\subsubsection{Constitutive restrictions for dissipative rates}
...
electrochemical.  The relative component fluxes redistribute mass among the
components of a phase while preserving the phase-total mass flux.  Adding the
same component potential to every component in a phase therefore leaves the
transport power unchanged.  The driving quantities are component-potential
differences, with the neutral theory recovered by setting \(z_\xi^\alpha=0\).
