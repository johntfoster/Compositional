...
current phase volume, so \(\phi_\xi\omega_\xi^+\) is its contribution per
current mixture volume.  The effective interfacial Helmholtz free-energy
density \(\gamma\) is normalized per current fluid volume, so
\(\phi\gamma\) is its contribution per current mixture volume.  The notation
\(\psi_\xi\) denotes the specific phase Helmholtz free energy,
\(\gamma\) the effective interfacial Helmholtz density, and
\(\omega_\xi^+\) the phase electric-enthalpy density.  The electric field is a single spatial field, but each phase
may have its own dielectric response to that field.  The derivative of
\(\omega^{+}_{\xi}\) with respect to \(\mbf E\) defines the phase displacement
\(\mbf d_\xi\), and the total displacement is
\(\mbf d=\sum_\xi\phi_\xi\mbf d_\xi\).  Stationarity with respect to
\(\varphi\) gives Gauss' law.  Thus \(\mbf E\) is the independent electrical
variable and \(\mbf d_\xi\) is the constitutive dielectric response.  This is
the electric-enthalpy form of electroelastic potential theory
\cite{dorfmann2017nonlinear_electroelasticity}.

Consistently with the variational state space,
\(\omega_f^+=\omega_f^+(\mbf E,\bar\rho_f,\{\eta_f^\alpha\},\theta)\).
Density, composition, and temperature therefore change the fluid permittivity
and its dielectric pressure.  Such state dependence is relevant
...
and temperature arguments.  These state arguments give the Maxwell stress and
the density, composition, and temperature contributions in the corresponding
...
consistency checks when the same free energies and constraints are used.  The
phase-map variation of the material-insertion constraint \eqref{eq:C4_def}
supplies the conversion-insertion term involving
\(\nabla_{\mbf x}\tau\) in \eqref{eq:el_momentum_uniform}; the Coleman--Noll
calculation uses that phase balance in the constitutive exploitation.
...
\eqref{eq:interfacial_energy_density}.  Allowing \(\gamma\) to depend on
temperature is consistent with standard capillary-pressure scaling used in
reservoir engineering \cite{leverett1941capillary}.  Its contribution per
current mixture volume and corresponding collective local rate satisfy
%
\begin{equation}
	\frac{{\rm d}}{{\rm d}t}
	\left[
...
\end{equation}
%
The adopted \(\gamma=\gamma(\{S_f\}_{f\in\mc F},\theta)\) makes capillary
coupling enter through the volume fractions, saturations, and temperature.  The derivative on the left of
\eqref{eq:MC_interfacial_helmholtz_rate} is the collective local constitutive
...
we take the energy equation as a balance-law postulate for the reacting
immiscible mixture \cite{drumheller1980thermomechanical}; its conversion-work
terms use the generalized form of Drumheller
\cite[eq.~(50)]{drumheller2000}.  The electrical extension is written in the
electric-enthalpy representation used in thermo-electroelasticity
...
stress defined by \eqref{eq:vp_electrical_stress_definition}, the postulated
energy equation is
%
\begin{equation}
	\begin{aligned}
	...
	\right)
	+
	\phi_\xi\omega_\xi^+\mbf I:\mbf L_\xi
	-
...
\end{equation}
%
Here \(e_\xi=\psi_\xi+\theta\mathfrak{s}_\xi\), and the two explicit
interfacial terms are precisely the collective rate of \(\phi\gamma\) in
\eqref{eq:MC_interfacial_helmholtz_rate}.  The total stress
\(\bs\sigma_\xi^{\prime+}\) appears with positive mechanical power.  When the
phase electric-enthalpy contribution \(\phi_\xi\omega_\xi^+\) in
\eqref{eq:MC_energy_balance} is expanded during the entropy-inequality
exploitation, its reversible Maxwell-power contribution is collected with
that total stress power, leaving the nonelectrical material stress power
identified below.
...
negative of the two contributions to the collective rate of \(\phi\gamma\) in
\eqref{eq:MC_interfacial_helmholtz_rate}.  The electrical terms come directly
from the phase electric-enthalpy contributions on the left-hand side of
...
displayed Gauss-law coefficient.  Expanding the phase electric-enthalpy terms
from \eqref{eq:MC_energy_balance} with
...
\(\phi\gamma\) is a collective interfacial Helmholtz contribution per current
mixture volume, this restriction defines the corresponding collective
interfacial contribution to the total entropy density:
%
\begin{align}
...
The following constitutive substitutions make the residual inequality
...
\end{equation}
%
The spherical part is orthogonal to the isochoric rate
...
\cite[eqs.~(108)--(112)]{drumheller2000}.  Starting from
\eqref{eq:MC_collected_entropy}, the residual inequality
...
the relative-flux constraint and make transport power invariant under a common
shift of the component potentials within a phase.  Substitution of
...
power is nonnegative.  This structure admits non-associated geomaterial
plasticity: the true-plastic rate and plastic-distension rate may use independent
directions, while their combined conjugate power in
\eqref{eq:MC_residual_dissipation} remains nonnegative.  Plastic distension can
therefore represent dilatancy, pore collapse, grain rearrangement, cementation,
dissolution, and precipitation through constitutive dependence on stress,
composition, reaction affinity, and internal state.
...
at fixed equivalent pressure, electric field, temperature, and composition,
%
\begin{align}
	\left.
	\p{\bar v_s}{\bar{\mbf F}_s^e}
	\right\vert_{\bar p_E,\theta,\{\eta_s^\alpha\},\mbf E}
	 & =
	\left.
	\p{\bar v_s}{J}
	\right\vert_{\bar p_E,\theta,\{\eta_s^\alpha\},\mbf E}
...
\begin{subequations}
	...
	\begin{align}
		...
		\label{eq:MC_phase_single_prime_pk_stress} \\
			...
			\left.
			\p{\psi_s}{\bar{\mbf F}_s^e}
			\right\vert_{\bar v_s,\{\eta_s^\alpha\},\theta,\mbf E},
