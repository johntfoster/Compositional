...
\end{equation}
%
For a mechanism confined to one thermal subsystem, the conversion power is
already \(\mc A_{(m)}\dot r_{(m)}/\theta_{(m)}\).  A fluid--solid mechanism
contains contributions divided by two different temperatures.  To express its
net conversion power with the chosen interfacial temperature
\(\theta_{(m)}=\theta_{\mc F}\), the accompanying interphase energy exchange
must supply the difference.

\pagebreak[3]
The required interphase energy allocation is
%
\begin{align}
	& -
...
\end{align}
%
With \eqref{eq:MC_reaction_energy_allocation}, the conversion and interphase
energy terms combine to
\(\mc A_{(m)}\dot r_{(m)}/\theta_{(m)}\).  This is the two-temperature analogue
of the coupled conversion and heat-exchange terms in Drumheller's constituent
entropy inequality \cite[eq.~(88)]{drumheller2000}.
...
are then replaced by the affinity and relative-transport powers derived in
\eqref{eq:MC_affinity_projection} and
\eqref{eq:MC_relative_transport_power}; fluid--solid conversion also uses the
energy allocation \eqref{eq:MC_reaction_energy_allocation}.  Gauss' law was
...
\sref{onsager_reaction_transport} supplies one near-equilibrium choice.  For
uncoupled mechanisms, it gives
%
\begin{equation}
	\dot r_{(m)}
	=
...
The remaining dissipation contains three processes: relative motion between
phases, heat exchange between the fluid and solid thermal subsystems, and heat
conduction within each phase.  Before closing these processes, conservation of
total mixture energy requires the internal-energy and momentum supplies to
satisfy \cite{drumheller1980thermomechanical}
%
\begin{equation}
	\sum_\xi
	\left(
...
\end{equation}
%
The reversible volume-constraint powers are not part of
\eqref{eq:MC_interaction_energy_conservation}; their coefficients have already
vanished through \eqref{eq:MC_volume_fraction_restrictions}.  The interaction
identity instead states that momentum exchanged between phases and the
associated change in internal energy conserve the total mixture energy.
...
\end{equation}
%
A sufficient uncoupled closure uses symmetric positive-definite drag tensors
\(\mbf k_{\xi\zeta}\), a nonnegative fluid--solid heat-transfer coefficient
\(h_{\mc F\mc S}\), and symmetric positive-semidefinite conductivity tensors
\(\mbf K_\xi\) \cite{foster2026reactingporous}:
%
\begin{subequations}
...
The transformed phase potential is
%
\begin{subequations}
...
correction into an isotropic Cauchy stress.  Therefore
%
\begin{empheq}[box=\fbox]{equation}
	\bs\sigma_s'
	=
...
\end{equation}
%
At reversible equilibrium, \(\mc A_{(m)}=0\), so the chemical
