...
\begin{equation}
	\mbf A_s^0
	=
	\mbf A_s^0(\theta,\{\eta_s^\alpha\}),
	\qquad
	\bar{\mbf F}_s^0
	=
	\bar{\mbf F}_s^0(\theta,\{\eta_s^\alpha\}).
...
\end{empheq}
%
For the held-fixed derivatives used next, \(\mc X_s\) denotes the solid state set
defined in \eqref{eq:MC_state_sets} below.  The intrinsic density rate used in
the Coleman--Noll collection is obtained from \eqref{eq:MC_solid_true_split},
the isochoric plastic flow, and the
...
Electrical effects enter the free-energy structure through the phase electric
enthalpies \(\omega^{+}_{\xi}\) introduced in
\sref{virtual_power_derivation}.  Their corresponding densities per current
mixture volume are
\(\psi_{\omega,\xi}\equiv\phi_\xi\omega_\xi^+\), in analogy with
\(\psi_\gamma=\phi\gamma\).  The electric field is a single spatial field, but
each phase may have its own dielectric response to that field, in the same
...
\(\varphi\) gives Gauss' law.  Thus \(\psi_\xi\) remains nonelectrical,
\(\mbf E\) is the independent electrical variable, and \(\mbf d_\xi\) is a
derived response.  This is the electric-enthalpy version of standard
electroelastic potential formulations, specialized here to exclude an
independent deformation argument in the electrical potential
\cite{dorfmann2017nonlinear_electroelasticity}.

Consistently with the variational state space, the arguments of
\(\omega_f^+\) are \(\mbf E\), \(\bar\rho_f\), the fluid mass fractions, and
\(\theta\), with no independent dependence on the full deformation gradient.
Density, composition, and temperature can therefore change the fluid
permittivity and its dielectric pressure.  Such state dependence is relevant
to reservoir materials: the bulk dielectric response of saturated sandstone
varies with porosity and pore-fluid
salinity~\cite{kirichek2018dielectric_sandstone}.  The solid enthalpy
\(\omega_s^+\) has the analogous field, intrinsic-density, solid-composition,
and temperature arguments.  Thus neither phase enthalpy has an independent
deformation-gradient argument, so no electrostrictive constitutive term is
introduced.  Density, composition, and temperature derivatives remain and
enter the matching Coleman--Noll restrictions.

We further assume that \(\omega_\xi^+\) is even in \(\mbf E\).  This excludes
a bulk piezoelectric coupling, which would require an electrical term odd in
\(\mbf E\).  Together with the restricted solid argument structure, this is a
reservoir-scale constitutive choice rather than a claim that every mineral grain
is nonpiezoelectric or nonelectrostrictive:
in saturated-rock hydraulic-fracturing measurements, Moore and Glaser found the
self-potential response to be primarily electrokinetic rather than
piezoelectric~\cite{moore2007self_potential}.  Streaming-current and
electro-osmotic couplings belong to the dissipative cross-coupled transport
sector, not to the equilibrium deformation derivative of \(\omega_\xi^+\).
Coreflood experiments in tight sandstone likewise identify electro-osmosis and
electrochemical alteration as principal mechanisms by which a direct-current
field changes pressure demand, wettability, and oil
recovery~\cite{jia2024electrically_enhanced_oil_recovery}.  The theory
therefore retains electrical coupling in phase equilibrium and charged-species
transport while omitting a direct electrical dependence of the solid plastic
constitutive law.  Electric loading can still influence plastic response
indirectly through the Maxwell stress in the coupled momentum equations.

The present model also omits independent polarization, dielectric relaxation,
and switching; adding those mechanisms would require additional electrical
...
separate electrical term.  The complete electrical stress
\(\bs\sigma_\xi^{+}\) is the Maxwell stress defined in
...
consistency checks when the same free energies and constraints are used, but the
Coleman--Noll calculation does not independently generate terms arising from
kinematic constraint variations.  In particular, the conversion-insertion
contribution involving \(\nabla_{\mbf x}\tau\) in the phase momentum balance
\eqref{eq:el_momentum_uniform} follows from the phase-map variation of the
material-insertion constraint \eqref{eq:C4_def} and is carried into the
constitutive development through the previously derived balance law.
...
\begin{equation}
	...
	\qquad
	\psi_f=\psi_f(\bar\rho_f,\{\eta_f^\alpha\},\theta).
...
\end{align}
%
We assume that the fluid phases carry no elastic stress,
%
\begin{equation}
	\bs\sigma_f'=\mbf 0,
	\qquad
...
therefore purely electrical.  Because the fluid state set has no independent
deformation-gradient argument, this contribution is Maxwellian:
%
\begin{equation}
	\bs\sigma_f^{\prime+}
	=
...
\end{equation}
%
As in the variational derivation, \(\gamma\) has no independent deformation
argument.  Capillary coupling therefore enters through the volume fractions,
saturations, and temperature rather than through a separate constitutive
capillary stress.  The overdot on \(\psi_\gamma\) denotes the collective local
rate generated by the independent fluid volume-fraction rates and the common
temperature rate in \eqref{eq:MC_interfacial_helmholtz_rate}; it is not a
material derivative following any one phase.  Because the present state space
contains no independent interface motion or interfacial-area density,
\(\psi_\gamma\) has no separate phase-transport term.  Such a term would
require an additional interfacial balance and flux law.
...
the internal energy supply from interphase exchange.  The electric-enthalpy
Legendre contribution \(\psi_{\omega,\xi}\) is also written per unit current
mixture volume, but unlike \(\psi_\gamma\), it is attributed to phase \(\xi\).
For a current volume element carried by \(\mbf v_\xi\), the spatial transport
theorem gives
\(D_\xi({\rm d}v)/Dt=(\nabla_{\mbf x}\cdot\mbf v_\xi){\rm d}v
=(\mbf I:\mbf L_\xi){\rm d}v\).  Applying the product rule gives
\eqref{eq:MC_electric_enthalpy_volume_transport}.  Similarly, differentiating
\(\mbf E=-\nabla_{\mbf x}\varphi\) with the moving phase derivative and using
\(D_\xi(\nabla_{\mbf x}\varphi)/Dt
=\nabla_{\mbf x}\dot\varphi-\mbf L_\xi^T\nabla_{\mbf x}\varphi\)
gives \eqref{eq:MC_electric_field_transport}.  Since
\(\mbf d_\xi=-\partial\omega_\xi^+/\partial\mbf E\), the chain rule for
\(\dot\psi_{\omega,\xi}\) contains
\(-\phi_\xi\mbf d_\xi\cdot\dot{\mbf E}\).  Substitution of the field-rate
identity produces a field-work term
\(\phi_\xi\mbf d_\xi\cdot\nabla_{\mbf x}\dot\varphi\) and a convected-field
term \(\phi_\xi\mbf E\otimes\mbf d_\xi:\mbf L_\xi\).  The field-work term is
moved to the left, while the convected-field term combines with the
current-volume transport term to give the Maxwell stress:
%
\begin{subequations}
	\label{eq:MC_electric_enthalpy_rate}
	\begin{align}
	\frac{D_\xi}{Dt}
	\left(
	\psi_{\omega,\xi}\,{\rm d}v
	\right)
	={} &
	\left(
	\dot\psi_{\omega,\xi}
	...
	\mbf I:\mbf L_\xi
	\right){\rm d}v,
	\label{eq:MC_electric_enthalpy_volume_transport}
	\\
	\dot{\mbf E}
	={} &
	...
	-
	\mbf L_\xi^T\mbf E,
	\label{eq:MC_electric_field_transport}
	\\
	\dot\psi_{\omega,\xi}
	+
	\psi_{\omega,\xi}\mbf I:\mbf L_\xi
	-
	\phi_\xi\mbf d_\xi\cdot\nabla_{\mbf x}\dot\varphi
	={} &
	...
	:\mbf L_\xi .
	\label{eq:MC_electric_enthalpy_rate_collected}
	\end{align}
\end{subequations}
%
Thus neither mechanical term is appended to the constitutive chain rule: the
isotropic term comes from transporting a current-volume density, and the
dyadic term comes from convecting the spatial electric field.  This is the same
bookkeeping used in
electric-enthalpy thermo-electroelasticity and in energy-balance derivations of
Maxwell stress \cite{montanaro2015thermo_electroelasticity,
nika2025heterogeneous_dielectrics}.

The interfacial energy cannot be assigned uniquely to an individual fluid
phase.  Therefore the first law used in the entropy inequality is the sum of
the phase contributions, with the collective interfacial rate added once.

\pagebreak
Using the generalized conversion-work potential \(L_\xi^\alpha\), this
electric-enthalpy form is
%
\begin{equation}
	\begin{aligned}
	& \sum_\xi
...
\end{equation}
%
Here \(e_\xi=\psi_\xi+\theta\mathfrak{s}_\xi\).
Equation~\eqref{eq:MC_energy_balance} displays both contributions to the single
collective rate \(\dot\psi_\gamma\) from
\eqref{eq:MC_interfacial_helmholtz_rate}; they do not belong to the
phase-attributed electrical identities in
\eqref{eq:MC_electric_enthalpy_rate}.
Equations~\eqref{eq:MC_electric_enthalpy_rate} and
\eqref{eq:MC_energy_balance} show why the signs are not opposite power
conventions: the total stress power is positive on the right of the first law,
and the positive electrical stress power is part of the electric-enthalpy
rate on the left.  It therefore enters the dissipation inequality with a minus
sign only after the stored-energy rate is moved to that inequality.
...
\end{align}
%
Eliminating \(\sum_\xi\rho_\xi r_\xi\) with \eqref{eq:MC_energy_balance}, adding the
...
The interfacial terms in \eqref{eq:MC_reduced_entropy_inequality} are the
negative of the two contributions to \(\dot\psi_\gamma\) in
\eqref{eq:MC_interfacial_helmholtz_rate}.  The electrical terms come directly
from the electric-enthalpy Legendre terms on the left-hand side of
\eqref{eq:MC_energy_balance}.  The electric-potential-gradient term has been
integrated by parts and combined with the free-charge power, giving the
displayed Gauss-law coefficient.  The total stress power has not changed sign:
it is the first positive term in
\eqref{eq:MC_reduced_entropy_inequality}.  The negative electrical stress power
is part of the stored electric-enthalpy rate
\eqref{eq:MC_electric_enthalpy_rate}.  Hence
\begin{equation}
	\bs\sigma_\xi^{\prime+}:\mbf L_\xi
	-
	\bs\sigma_\xi^+:\mbf L_\xi
...
\end{equation}
This cancellation identifies the portion conjugate to the separately defined
nonelectrical Helmholtz energy \(\psi_\xi\); it does not remove electrical
stress from the momentum equation.  Derivations based on the Poynting flux
obtain the same sign: the first law contains the total stress power, whereas
the field-energy transport supplies the Maxwell-stress power that is
subtracted when the Clausius--Duhem inequality is formed
...
\(\bs\sigma_\xi^{\prime+}\) remains the sum of this nonelectrical Helmholtz
stress and the complete electrical stress \(\bs\sigma_\xi^+\), which is
Maxwellian under the constitutive assumptions of this theory.  This bookkeeping
does not imply that a fluid has no electrical stress.
...
\eqref{eq:MC_fluid_stress_free}; therefore the same restriction reads
\(\bs\sigma_f^{\prime+}=\bs\sigma_f^+\), not
\(\bs\sigma_f^{\prime+}=\mbf0\).  The electric stress remains in the fluid
momentum balance \eqref{eq:el_mom_f_simplified}; only its reversible power is
matched by the electric-enthalpy rate in the entropy inequality.
...
\(\bs\sigma_s^+\) as the phase-attributed Maxwell stress.  No separate
electrostrictive stress is present because \(\omega_s^+\) has no independent
deformation-gradient argument.
...
\end{empheq}
%
The temperature coefficient gives the total entropy restriction.  Because
\(\psi_\gamma\) is a collective mixture contribution, this restriction fixes
the total entropy density but does not prescribe an arbitrary allocation of
interfacial entropy among the fluid phases:
%
\begin{align}
...
thermodynamic component potential that must be conjugate to component supply.
We make the admissible choice
%
\begin{equation}
	L_\xi^\alpha
	=
...
retained.  The source contribution in \eqref{eq:MC_collected_entropy} therefore
separates into reaction and relative-transport powers by substituting
...
\(\partial\mc B\).  The local entropy-production contribution is therefore
%
\begin{equation}
	-\sum_\xi\sum_\alpha
	\left(
...
With electrical effects, the force conjugate to the component rate in
...
\(\varphi\) is the component charge per unit mass \(z_\xi^\alpha\), because the
rate balance is written per unit component mass.  Thus the same local reaction
and transport powers apply with \(\mu_\xi^\alpha\) interpreted as the
electrochemical
component potential:
%
\begin{align}
...
\end{empheq}
%
Equation \eqref{eq:MC_admissible_conversion_component} follows from varying the
accumulated component measure in the constrained Hamilton--d'Alembert statement
and evolves the single Bedford--Drumheller transfer potential \(\tau\) using the
reference component.  Equation
\eqref{eq:transfer_potential_component_compatibility} then constrains the
remaining \(N-1\) component storage multipliers relative to component \(N\).
...
well-defined.
%
\subsubsection{Residual dissipation and admissible closures}
We now collect the constitutive substitutions that make the residual inequality
nonnegative.  The reaction and relative-transport terms use the Onsager
closures derived in \sref{onsager_reaction_transport}; the remaining terms are
closed by standard sufficient dissipative choices.
...
\eqref{eq:gauss_law}.  The complete electrical power is accounted for by the
electrical enthalpy rate; under the present specialization this is the Maxwell
power.  The residual plastic powers therefore contain \(\bs\sigma_s'\).  The
source and transport reductions are
...
\end{equation}
%
The residual inequality \eqref{eq:MC_residual_dissipation} is the admissibility
check for the Onsager laws obtained in \sref{onsager_reaction_transport}.  The
reaction substitution is
%
\begin{equation}
	\mc A_{(m)}
	=
...
\end{equation}
%
The relative-transport substitutions are
%
\begin{align}
	\sum_\beta
...
\end{align}
%
Here \(k\in\{{\rm disp},{\rm diff}\}\).  The zero row and column sums enforce
the relative-flux constraint, so adding a common component potential within a
phase does not produce transport power.  Substitution of
...
\end{align}
%
Thus the reaction and relative-transport terms are dissipative provided
\(K_{(m)}>0\) and each \(\mbf D_{\xi,k}^{\alpha\beta}\) is symmetric positive
definite on the relative-flux subspace satisfying
\eqref{eq:averaged_relative_flux_sum}.  Diffusion and dispersion may use
different mobility tensors, but both are driven by the same component-potential
gradient.  In the electrical theory \(\mu_\xi^\alpha\) is electrochemical; the
neutral theory is recovered by setting \(z_\xi^\alpha=0\).  The mobility tensors
are constitutive choices, and dispersion may depend on resolved flow and pore
geometry~\cite{plumb1988dispersion,gray2009tcat_transport}.  Cross-coupled
mobilities can be added when the material requires them, provided the combined
quadratic form remains nonnegative~\cite{doi2011onsager,wang2021onsager}.
...
The remaining dissipative terms in \eqref{eq:MC_residual_dissipation} are the
plastic powers, the interphase interaction power, and the heat-conduction power.
...
dependence of the plastic closure is admissible at this residual level.  With
the internal-variable description, composition-dependent hardening, softening,
...
\(\{\eta_s^\alpha\}\).  Component-resolved plastic closures for the additive
...
non-associatively because shear strength and volumetric evolution are controlled
by different mechanisms, including dilatancy, pore collapse, grain
rearrangement, cementation, dissolution, and precipitation.  Imposing associated
flow on a frictional material can therefore predict an unrealistic volumetric
response.  The mixture formulation expresses this physical separation through
the distension split, with the yield-surface and plastic-potential roles
replaced by thermodynamic conjugate powers.  The elastic stress follows from the
...
\eqref{eq:MC_residual_dissipation} is nonnegative.  Plastic distension may
therefore be driven by hydrostatic stress, composition, reaction affinity,
cementation state, or other internal variables, provided the resulting coupled
deformation--distension power remains nonnegative.  The formulation thereby
retains the physical content usually associated with non-associated flow in
geomechanics while using the residual inequality as the admissibility test.

This distinction is important because empirical usefulness alone is insufficient
to establish thermodynamic consistency.  Separating the plastic flow direction from
the yield or loading function affects the plastic stress power, the usual
maximum-dissipation interpretation, and often the symmetry of the algorithmic
tangent.  The present formulation makes this check explicit by displaying the
true-plastic and plastic-distension powers in
\eqref{eq:MC_residual_dissipation}.  A non-associated closure is admissible only
after its chosen rates are tested against those conjugate power terms.  The
uncoupled rules in \eqref{eq:MC_generic_plastic_flow_rules} give one sufficient
construction, while more general coupled rules must satisfy the same
...
\begin{subequations}
	...
	\begin{gather}
		...
		\right\vert_{\bar{\mbf F}_s^e,\mbf A_s^e,\{\eta_s^\alpha\},\theta}.
...
\begin{align}
	...
	\right\vert_{\bar p_s,\theta,\{\eta_s^\alpha\}}
	 & =
	\left.
	\p{\bar v_s}{J}
	\right\vert_{\bar p_s,\theta,\{\eta_s^\alpha\}}
...
\begin{subequations}
	...
	\begin{align}
		...
		\left.
		\p{\psi_s}{\bar{\mbf F}_s^e}
		\right\vert_{\bar v_s,\{\eta_s^\alpha\},\theta},
...
\(\bs\sigma_s'\), because the complete electrical
power has already been accounted for by the electrical enthalpy in the residual
inequality.  If a plastic model is calibrated in terms of
\(\bs\sigma_s^{\prime\prime}\), the phase split
\eqref{eq:MC_phase_biot_split} must be applied before evaluating the plastic
driving stress.  Thus the constitutive assumptions of the present theory
contain no direct electroplastic coupling; the Maxwell stress can nevertheless
change \(\bs\sigma_s'\) indirectly through the coupled momentum solution.
...
plastic rates.  Only the summed mobilities enter the phase plastic powers, so
the sums must be nonnegative;
