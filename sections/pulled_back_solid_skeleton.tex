...
+\sum_\alpha\dot{c}_f^\alpha\mbf I\)
has eigenvalues
\(\widetilde{\lambda}_{f,i}=\phi_f^2\mu_f^{\mathrm{visc}}/\kappa_{f,i}
+\sum_\alpha\dot{c}_f^\alpha\),
for each \(f\in\mc F\).  The entropy-inequality admissibility conditions
require these eigenvalues to be positive, so the combined resistance is
positive definite and the mobility \(\widetilde{\bs\lambda}_f\) is well
