...
\end{equation}
%
where $\mu_f^{\mathrm{visc}}$ is the dynamic viscosity and $\bs\kappa_f$ is the permeability
tensor for phase $f$.  Phase relative permeability may be absorbed into
$\bs\kappa_f$.  Equation~\eqref{eq:fluid_phase_drag_law} is a drag-only
specialization of the interaction force.  Positive definiteness of
\(\bs\kappa_f\) controls the drag-generated dissipation, but complete
thermodynamic admissibility also requires the interaction-energy allocation to
satisfy \eqref{eq:MC_interaction_energy_conservation} and
\eqref{eq:MC_bulk_transport_interaction_admissibility}.  When additional
reversible interaction forces are required, \eqref{eq:fluid_phase_drag_law}
represents only the drag contribution; the full \(\mbf b_f\) remains in
\eqref{eq:el_mom_f_simplified}, and its non-drag contribution must be added to
the right-hand side of the reduction below.
...
For the drag-only specialization, the resulting relative mass flux is
%
...
separate from the role of \(\tau\) in the conversion variation.  The
...
replaces the joint
transport--interaction condition
...
net conversion phase-mass sources are large enough for momentum production to
