...
\(\phi_f>0\).  These conditions make the drag contribution nonnegative in
the entropy inequality.  The added scalar term comes from conversion insertion
in the momentum balance and supplies no independent quadratic dissipation.
Because \(\bs\kappa_f\) is symmetric with eigenvalues
\(\kappa_{f,i}\), the combined resistance
\(\phi_f^2\mu_f^{\mathrm{visc}}\bs\kappa_f^{-1}
+\sum_\alpha\dot{c}_f^\alpha\mbf I\)
has eigenvalues
\(\phi_f^2\mu_f^{\mathrm{visc}}/\kappa_{f,i}
+\sum_\alpha\dot{c}_f^\alpha\),
for each \(f\in\mc F\).  Algebraic elimination of \(\mbf w_f\) requires
exactly that none of these shifted eigenvalues vanish,
%
\begin{equation}
	\sum_\alpha\dot{c}_f^\alpha
	\ne
	...
	\qquad
	i=1,\ldots,d,\quad f\in\mc F .
	\label{eq:modified_relative_flux_nonsingularity_condition}
\end{equation}
%
This nonsingularity condition defines \(\widetilde{\bs\lambda}_f\).  A stronger
positive-resistance regime requires every shifted eigenvalue to be positive.
If \(\kappa_{f,\max}\) and \(\kappa_{f,\min}\) are the largest and smallest
eigenvalues of \(\bs\kappa_f\), respectively, the binding direction is that of
largest permeability, and the positive-resistance condition is
%
\begin{equation}
	\sum_\alpha\dot{c}_f^\alpha
	>
	...
	\qquad
	f\in\mc F .
	\label{eq:modified_relative_flux_positive_resistance_condition}
\end{equation}
%
...
\end{equation}
%
Thus quantitative conditioning within this regime requires the smallest
shifted eigenvalue in the denominator to remain bounded away from zero.
...
equality it is singular in the largest-permeability direction.  At shorter
time scales it leaves the positive-resistance regime, while algebraic inversion
remains possible except at the individual eigenvalue-crossing times
\(t_{\rm rxn}=\rho_f\kappa_{f,i}/
(\phi_f^2\mu_f^{\mathrm{visc}})\).
...
\(\tau\).  Thermodynamic dissipation resides in the drag and
component-transport closures.  The nonsingularity condition permits algebraic
flux elimination, and the positive-resistance condition selects the stronger
regime used for the conditioning estimate.  The interaction-energy allocation
of \eqref{eq:MC_drag_heating_equal_partition} is retained.
