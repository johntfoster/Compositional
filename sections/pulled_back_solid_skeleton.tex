...
The dynamic-capillary phase momentum equation
...
phase-pressure gradient including equilibrium and dynamic capillarity, Maxwell
stress, and conversion insertion.
...
The same scalar-gradient pull-back carries the dynamic-capillary term through
...
electrocapillary, pressure closes the phase pressures at the current saturation
state and saturation rate,
%
\begin{equation}
...
evaluating the fluxes.  Because \(\mbf X\) follows the skeleton,
\(\left.\partial S_n/\partial t\right\vert_{\mbf X}=D_{\mc S}S_n/Dt\).
The neutral equilibrium specialization
\(\omega_f^+=0\) and \(\mc T=\mbf0\) gives
\(p_{c,nw}=\bar p_n-\bar p_w\).  In a fully implicit implementation, the
time-discrete saturation rate enters the pressure reconstruction, and its
spatial gradient is carried by the Darcy flux.  Together with the Maxwell
stress, the pressure reconstruction gives the equivalent-pressure identity
and dynamic term in \eqref{eq:MC_fluid_pressure_gradient_rollup}.
...
reconstructions, and
\eqref{eq:reservoir_simulation_capillary_summary} is a history- and
rate-dependent
constitutive closure.  A
