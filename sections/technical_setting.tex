...
prescribed, and \(\Gamma_{\bar q}\), where
...
boundary problem,
\(\Gamma_{\bar q}=\partial\mc B\), solvability and its fixed-domain rate require
%
\begin{subequations}
...
reference state used here takes the transfer potential to be initially
uniform.  Its time-independent additive constant is fixed by choosing
%
\begin{equation}
	\tau(\mbf x,0)=0,
	\qquad
...
The unresolved interfacial entropy follows the skeleton trajectory and has no
separate boundary flux, so it introduces no additional thermal boundary datum.
