...
The signs of the conversion-work terms in \(\delta W\) fix the thermodynamic
force convention used throughout the paper.  The variables
\(\mc C_\xi^\alpha\) and \(\dot r_{(m)}\) are internal conversion coordinates,
not externally prescribed displacement coordinates.  Thus
\(L_\xi^\alpha\) and \(\mc A_{(m)}\) are written as internal generalized forces
conjugate to insertion of component mass and to mechanism progress,
respectively, and their virtual work enters with the opposite sign from the
corresponding virtual conversion.  With the stoichiometric convention in
\eqref{eq:C4_def}, this choice identifies the mechanism affinity as minus the
stoichiometric projection of the component conversion potentials.  A forward
reaction that lowers the appropriate thermodynamic potential then has positive
affinity, and admissible kinetics make the product
\(\mc A_{(m)}\dot r_{(m)}\) nonnegative in the residual dissipation inequality.
One could reverse the signs only by redefining the conversion potentials and
affinities everywhere downstream; the present convention is chosen so that
positive affinity times positive progress represents dissipated thermodynamic
driving power.
...
Collecting the coefficients of the variations $\delta\mbf{x}_\xi$, $\delta\phi_\xi$, $\delta\bar\rho_\xi$, $\delta\eta^\alpha_\xi$, and $\delta\mc{C}^\al_\xi$, together with the independent process-rate variations $\delta\dot r_{(m)}$, gives the following collection before substituting any Euler--Lagrange equation back into another variation.
%
% AGENT-LOCK-BEGIN equation-lock:  do not edit unless explicitly unlocked by user
\begin{align}
...
$\dot r_{(m)}=\mc K_{(m)}(\mc A_{(m)}, \bs{\Lambda})$, where \(\bs{\Lambda}\) is a vector of internal state variables.  Thus, for kinetically limited
