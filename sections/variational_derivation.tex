...
\begin{equation}
	...
	+\left(1-B_s^s\right)\bar p_s\mbf I .
...
\begin{align}
	...
	\Aboxed{B
	 & =
		1-\sum_{s\in\mc S}\left(1-B_s^s\right)} .
...
\(B_s^s\) is defined from the partial Piola stress by multiplication with the
...
therefore reduces to \(B_s=B_s^s\), recovering the standard Biot form.
...
poromechanics.  Since
$\phi\gamma\left(\left\{S_f\right\}_{f\in\mc F},\theta\right)$ is homogeneous
of degree one in the primitive fluid volume fractions, Euler's theorem gives
$\phi\gamma=\sum_{f\in\mc F}\phi_f\gamma_f$, or $\gamma=\sum_{f\in\mc F}S_f\gamma_f$.  Taking the
differential of this Euler relation and subtracting the defining differential
$d(\phi\gamma)=\sum_{f\in\mc F}\gamma_f\,d\phi_f$ gives the Gibbs--Duhem identity
$\sum_{f\in\mc F}\phi_f\,d\gamma_f=0$ \cite{callen1985thermodynamics}.  In gradient form,
$\sum_{f\in\mc F} S_f\nabla_{\mbf x}\gamma_f=\mbf 0$.  Therefore, since
$\bar p_f=\gamma_f+\bar p_E$,
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
	\sum_{f\in\mc F} S_f\nabla_{\mbf x}\bar p_f
