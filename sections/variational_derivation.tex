...
$\phi_l$.  For thermal reservoir simulation the interfacial energy is allowed
to depend on temperature, consistent with the standard capillary-pressure
scaling by interfacial tension in reservoir engineering
\cite{leverett1941capillary}.  The effective mobile interfacial energy is
written as a sum over wetting/non-wetting pairs,
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
	\gamma\left(\left\{S_l\right\}_{l\in\mc M},\theta\right)
	=\sum_{(w,n)\in\mc I}
	\gamma_{wn}\left(\left\{S_l\right\}_{l\in\mc M},\theta\right).
...
\[
	\delta_x\phi_l
	= -\nabla_{\mbf x}\cdot(\phi_l\delta\mbf x_l),
\]
% AGENT-LOCK-END
which is used below.
...
\begin{align}
	...
	\gamma_l\nabla_{\mbf x}\cdot(\phi_l\delta\mbf x_l)\dv \notag                                      \\
	& = \int_{\mc B}\sum_{l\in\mc M}\gamma_l\delta\phi_l\dv \notag                                    \\
	& \quad
	+\int_{\mc B}\sum_{l\in\mc M}
	\phi_l\nabla_{\mbf x}\gamma_l\cdot\delta\mbf x_l\dv
	+ \text{boundary terms}.
	\label{eq:deltaU_fg}
\end{align}
% AGENT-LOCK-END
...
wetting/non-wetting interfacial difference associated with an oriented pair is
%
% AGENT-LOCK-BEGIN equation-lock: fixed through rendered equation (49); do not edit unless explicitly unlocked by user
\begin{equation}
...
\begin{align}
	...
	\Bigg]\cdot\delta\mbf{x}_{\xi} \,\dv \notag                                                                                                              \\
	    & - \int_{\mc B}\Big[
		\sum_{l\in\mc M}
		\phi_l\nabla_{\mbf x}\gamma_l\cdot\delta\mbf x_l
	\Big]\dv \notag                                                                                                                                          \\
	    & + \sum_\xi \int_{\mc{B}} \Bigg[
...
reduces to the usual two-phase capillary pressure.  If the pair energy is
parameterized by the wetting saturation $S_w$, then
$p_{c,nw}=-\partial\gamma_{wn}/\partial S_w$.
...
For any mobile phase $l\in\mc M$, the capillary contribution gives the
intermediate form
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{align}
...
$\phi_l\nabla\gamma_l$, not merely algebraic multiples of
$\nabla\phi_l$.  These terms must be retained until the volume-fraction
pressure relations are substituted.  For a mobile phase,
$\lambda+\bar p_l-\gamma_l=0$, so
$\phi_l(\nabla\lambda-\nabla\gamma_l)=-\phi_l\nabla\bar p_l$.  The
