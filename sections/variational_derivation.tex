...
\begin{equation}
	\bar p_s
	=-\left.\p{\psi_s}{\bar v_s}\right\vert_{\bar{\mbf F}_s^e,J,\theta}.
...
\begin{subequations}
	...
	\begin{align}
		...
		\left.\p{\widetilde\psi_s}{\bar p_s}\right\vert_{\bar{\mbf F}_s^e,J,\theta}
		 & =\bar v_s .
...
\eqref{eq:CN_component_intrinsic_pressure_definition} removes the isotropic density-power
term before the elastic-distension coefficient is set to zero.  The remaining
restriction \eqref{eq:CN_distension_force_definition} is tensorial.  Thus the
...
it does not add $\bar p^{s_{(m)}}_c$ as a mechanical load.
...
\eqref{eq:el_momentum_uniform}, with the fluid-phase pressure-gradient
specialization shown in \eqref{eq:el_mom_f_simplified}, or summed into
the overall poromechanics form \eqref{eq:overall_momentum_nonlinear_biot}.  The
algebraic volume and fluid-composition constraints are
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{subequations}
...
\begin{subequations}
	...
	\begin{align}
		\lambda+\bar p_f-\gamma_f
		 & =0,
		\qquad f\in\mc F,
		\label{eq:model_summary_fluid_pressure_relation} \\
			\lambda+\bar p_s
		 & =0,
		\qquad s\in\mc S,
		\label{eq:model_summary_solid_pressure_relation} \\
		\sum_\alpha\frac{\tau_f^\alpha}{\phi_f}
		 & =\bar p_f,
		\qquad f\in\mc F,                                \\
		\frac{\tau_s^s}{\phi_s}
		 & =\bar p_s,
...
the redundant normal multiplier $\pi_f$ eliminated explicitly.  For fluid phases
...
Let $P_F=|\mc F|$ be the number of fluid phases, $P_S=|\mc S|$ the number of
solid phases, $P=P_F+P_S$ the total number of phases, $N$ the number of fluid
components tracked in each fluid phase, $N_{(m)}$ the number of independent
...
\begin{center}
	\begin{tabular}{p{0.72\textwidth}c}
		...
		Fluid mass fractions $\eta_f^1,\ldots,\eta_f^N$ for $f=1,\ldots,P_F$              & $P_FN$     \\
		Phase volume fractions $\phi_\xi$ for $\xi=1,\ldots,P$                            & $P$        \\
		Volume-fraction multiplier $\lambda$                                              & $1$        \\
		Storage multipliers $\tau_f^\alpha$ and $\tau_s^s$                                & $P_FN+P_S$ \\
		Helmholtz chemical potentials $\mu_f^\alpha$ and $\mu_s^s$                        & $P_FN+P_S$ \\
...
\begin{equation}
	\begin{aligned}
		N_{\rm unk}
		 & =dP+P+P_FN+P+1+\left(P_FN+P_S\right)
		+\left(P_FN+P_S\right)+N_{(m)}+2d^2P_S+1 \\
		 & =dP+2d^2P_S+3P_FN+2P_S+2P+N_{(m)}+2 ,
	\end{aligned}
...
\begin{center}
	\begin{tabular}{p{0.62\textwidth}c}
		...
		Projected fluid composition stationarity \eqref{eq:CN_composition_projected_restriction} & $P_F(N-1)$ \\
		Fluid composition normalizations in \eqref{eq:model_summary_constraints}                 & $P_F$      \\
		...
		Component mass balances \eqref{eq:model_summary_mass_balance}                            & $P_FN+P_S$ \\
		Conversion-potential equations \eqref{eq:el_conversion_component}                        & $P_FN+P_S$ \\
...
\begin{equation}
	\begin{aligned}
		N_{\rm eq}
		 & =dP+P+P_F(N-1)+P_F+P+1+\left(P_FN+P_S\right) \\
		 & \quad
		+\left(P_FN+P_S\right)+N_{(m)}+2d^2P_S+1        \\
		 & =dP+2d^2P_S+3P_FN+2P_S+2P+N_{(m)}+2
		=N_{\rm unk}.
...
$\sum_\alpha\eta_f^\alpha=1$, $\mc A_{(m)}$ is substituted from
