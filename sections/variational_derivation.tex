...
as an aqueous, oleic, or gas phase.  The symbol $s \in \mc{S}$ denotes solid phases and $f \in\mc F$ denotes fluid phases, while $\xi$ ranges over all phases.  Components within each phase are indexed by $\alpha$.
...
% AGENT-LOCK-END
%
For the fluid phases the corresponding equations contain the primitive
...
% AGENT-LOCK-END
%
We recognize the right-hand side as the thermodynamic phase pressure $\bar{p}_\xi$
and define
%
\begin{equation}
	\boxed{\bar{p}_\xi = \bar\rho^2_{\xi}\left.\p{\psi_{\xi}}{\bar\rho_{\xi}}\right\vert_{\mbf F_\xi,\eta_\xi^\beta}}
	\label{eq:phase_pressure_all}
\end{equation}

Let $f\in\mc F$ denote a fluid phase.
...
\(J\).  While each solid phase $s$ follows the same trajectory, they are allowed to each have distinct elastic, plastic, and thermal responses to that motion.

Extending Drumheller~\cite{drumheller2000} and Foster et.\ al~\cite{foster2026reactingporous} we introduce a multiplicative split of the deformation gradient into a \emph{distention} gradient and a true deformation gradient.  Distension is introduced to delineate pore space/archictecture changes from deformation that stresses the mineral solid phase. The common
...
% AGENT-LOCK-END
%
A subtle issue with the Terzaghi effective stress $\bs\sigma^\prime$ is that it should be populated with material properties taken at fixed $\bar\rho_s$.  This is a difficult, if not impossilbe experiment to perform.  It's more common to perform experiments on dry samples taken at fixed $\bar{p}_s = \bar{p}_E$ in the laboratory.  This leads us to pursue a transformed Biot effective stress $\bs\sigma^{\prime\prime}$.

We start by defining some convience variables for the various sspecific densites
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
	...
	v_s^s=\frac{1}{\rho_s},
	\qquad
	v_{s0}^s=\frac{1}{\rho_{s0}^s},
	\qquad
	\bar v_s^s=\frac{1}{\bar\rho_s^s},
	\qquad
	\bar v_{s0}^s=\frac{1}{\bar\rho_{s0}^s}.
\end{equation}
% AGENT-LOCK-END
%
...
\(\bar v_s^s=\bar v_s^s(J,\bar p_s^s,\theta)\) for this calculation, with \(a_s\),
\(\bar{\mbf F}_s^p\), and \(\theta\) held fixed.  Thus
\(\bar{\mbf F}_s^\theta\) is also fixed in the derivative below.
Therefore
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{align}
	\left.\p{\bar v_s^s}{\bar{\mbf F}_s^e}\right\vert_{\bar p_s^s,\theta}
	 & =
	\left.\p{\bar v_s^s}{J}\right\vert_{\bar p_s^s,\theta}
...
% AGENT-LOCK-END
%
At fixed \(\bar p_s^s\), the Legendre transform gives the fixed-pressure
Helmholtz derivative
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
	\rho_{s0}^s
	\left.\p{\psi_s}{\bar{\mbf F}_s^e}\right\vert_{\bar p_s^s,\theta}
	=
	\rho_{s0}^s
...
\end{equation}
%
Using \(\bar{J}_s \bar\rho_s=\bar\rho_{s0}^s\), this is the reference stress
...
% AGENT-LOCK-END
%
Notice the factor $a_s$ that appears in \eqref{eq:component_nonlinear_biot_coefficient}.  This is the distension factor that maps the common skeleton Jacobian to the true solid Jacobian.  The derivative is taken at fixed solid-phase pressure, so that the Biot coefficient is a measure of the change in true solid volume fraction with respect to the common skeleton volume fraction.  The distension factor $a_s$ is therefore required to map the derivative to the common skeleton volume fraction.  For any nonreacting formulation, it's not necessary to peform the split into distension and true deformation, and $a_s=1$.  In that case, the Biot coefficient reduces to the standard nonlinear form derived in \cite{foster2025} where is was shown to be equivalent to the standard Biot coefficient in the linear limit, i.e.\ $B_s = 1 - \nicefrac{K_s^\star}{K_s}$ where $K_s^\star$ is the drained bulk modulus of the solid skeleton and $K_s$ is the bulk modulus of the solid mineral grains.  The distension factor $a_s$ therefore generalizes the Biot coefficient to account for changes in pore space allocation, packing, and stress-free volume accommodation between the common skeleton motion and the true solid deformation.

Substituting \eqref{eq:CN_intrinsic_component_stress} and \eqref{eq:solid_pressure_equivalent_component_average} the momentum balance for a single solid phase is
%
\begin{equation}
	\begin{aligned}
		\rho_s\mbf a_s
...
effective stress $\sigma^{\prime}$.  Therefore, if a constitutive plasticity model is
specified in terms of $\sigma^{\prime\prime}$ the stress must first be mapped
...
% AGENT-LOCK-END
%
It's common in poromechancis to solve the ``overall'' momentum balance by summing the phase momentum balances over all phases.  In preparation for this step, and to develop mixture rules for the phase level Biot coefficients, we substitute \eqref{eq:solid_pressure_equivalent_component_average} into the Coleman--Noll mixture rule and sum over all solid phases gives the aggregate solid stress
split
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{align}
...
reference partial density \(\rho_{s0}^s\).  In the single-solid, single-fluid
