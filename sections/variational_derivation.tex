...
\(\phi_s\) appears in \eqref{eq:component_biot_stress_split}, because
...
% AGENT-LOCK-END
%
Summing the single-solid balance \eqref{eq:single_solid_phase_momentum} with
the corresponding fluid-phase balances and using
\eqref{eq:fluid_pressure_gradient_equivalent_pressure} gives
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
...
\eqref{eq:solid_biot_mixture_stress_definition} gives the following compact
form.  In the conversion-momentum terms, the component sum is over the
admissible components of each phase; for \(s\in\mc S\), the one-component solid
specialization gives only the contribution with \(\eta_s^s=1\).
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
	\sum_\xi\rho_\xi\mbf a_\xi
...
exactly the Biot pressure correction in \eqref{eq:component_biot_stress_split}.
