...
common true Jacobian.  Define the partial first Piola--Kirchhoff stress by
%
\begin{equation}
	\mbf P_s'
	=
...
% AGENT-LOCK-END
%
Using \(\bar{J}_s \bar\rho_s=\bar\rho_{s0}^s\), the partial stress has the standard Piola form
inside the distension map:
%
\begin{equation}
	\bs\sigma_s'
	=
...
momentum balances, using the fluid-phase pressure relations, and retaining the
...
\begin{equation}
	\begin{aligned}
		...
		-\sum_{f\in\mc F}\phi_f\nabla_{\mbf x}\bar p_f                                                     \\
		    & +\sum_{f\in\mc F}\sum_\alpha\dot c_f^\alpha\left(\nabla_{\mbf x}\mu_f^\alpha-\mbf v_f\right)
...
% AGENT-LOCK-END
%
where \(f\) ranges over the fluid pore phases and \(\rho=\sum_\xi\rho_\xi\).
This is the pressure form of the capillary force.  The
primitive potentials $\gamma_f$ are useful in the variation, but the
volume-fraction Euler--Lagrange relation and the component-storage multiplier
relation combine through \eqref{eq:fluid_capillary_pressure_gradient_collapse}.
...
% AGENT-LOCK-END
%
The first term on the right is the single-prime partial first
Piola--Kirchhoff stress defined in
\eqref{eq:solid_component_partial_pk_stress_definition}.  The fixed-pressure
counterpart is
%
% AGENT-LOCK-BEGIN equation-lock: do not edit unless explicitly unlocked by user
\begin{equation}
