...
At fixed temperature, let the specific phase Helmholtz free energy have the
...
\end{align}
%
Unlike the corresponding terms in an energy-valued Hamilton integrand, every
pairing in \eqref{eq:vp_dissipative_power} is a virtual power.
...
constraints is derived once here.  Consider a one-parameter family of phase
motions \(\mbf x_\xi^\epsilon(\mbf X_\xi,t)\) and spatial volume-fraction
fields \(\phi_\xi^\epsilon(\mbf x,t)\).  Pulling the spatial field back to the
phase reference configuration gives
%
\begin{equation*}
	\phi_\xi^\epsilon
	\left(
	\mbf x_\xi^\epsilon(\mbf X_\xi,t),t
	\right).
\end{equation*}
%
Differentiating this composition with respect to \(\epsilon\) at fixed
\(\mbf X_\xi\) and evaluating at \(\epsilon=0\) gives
%
\begin{align*}
	\left.
	\frac{d}{d\epsilon}
	...
	={}&
	\left.
	\frac{d\phi_\xi^\epsilon}{d\epsilon}
	\right\vert_{\mbf x,\epsilon=0}
	+
	\nabla_{\mbf x}\phi_\xi\cdot
	\left.
	\frac{d\mbf x_\xi^\epsilon}{d\epsilon}
	\right\vert_{\mbf X_\xi,\epsilon=0}
	\\
	={}&
...
\end{align*}
%
The reference-side derivative is the phase-attached virtual material rate
\(\delta\dot\phi_\xi\), while the motion derivative is the virtual phase
velocity \(\delta\mbf v_\xi\).  Solving for the fixed-spatial variation and
pushing the result forward to the current configuration gives
%
\begin{equation*}
	\left.\delta\dot\phi_\xi\right\vert_{\mbf x}
	=
...
\end{equation*}
%
The pointwise constraint is \(\sum_\xi\phi_\xi=1\), so its fixed-spatial
virtual rate is
%
\begin{equation}
	\sum_\xi
...
The flux contribution is integrated by parts before it is virtualized,
%
\begin{align}
...
\end{align}
%
For compactness in the momentum equations, define the total current-volume
...
where they multiply the same momentum factor; the two rates remain separate in
their balance and dissipation equations.
