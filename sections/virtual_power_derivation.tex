...
\({\rm in}\), \({\rm ext}\), \({\rm rev}\), \({\rm diss}\), and \({\rm elec}\)
denote inertial, external, reversible, dissipative, and electrical virtual
power, respectively, while \(C_k\) denotes the constraint contributions.  The
statement is time-integrated as in the Bedford--Drumheller constrained Hamilton
formulation, whereas each term in its integrand is expressed as a virtual power.
The electrical virtual power and \(C_5\) are absent in the neutral
specialization.
...
\subsection{Electrical virtual power}

The electric-field contribution is a reversible power, not a component-balance
constraint.  In the electroquasistatic specialization, the electric potential
\(\varphi\) generates a single spatial electric field.  Each phase has a
current-volume electric enthalpy \(\omega^{+}_{\xi}\), just as each phase has a
Helmholtz free energy \(\psi_\xi\),
%
\begin{equation}
	\mbf E=-\nabla_{\mbf x}\varphi,
	\qquad
	...
	=
	-\left.
	\frac{\partial\omega^{+}_{\xi}}{\partial\mbf E}
...
\end{equation}
%
Here \(\omega^{+}_{\xi}\) is an electric enthalpy per unit current volume of
phase \(\xi\), and \(\mc X_\xi\) denotes the corresponding phase, compositional,
mechanical, and thermal state variables on which the dielectric closure may
depend.  The total current-volume electrical enthalpy is
\(\sum_\xi\phi_\xi\omega^{+}_{\xi}\), and the total electric displacement is the
volume-fraction-weighted sum in \eqref{eq:electric_field_definitions}.  A
linear dielectric is recovered by choosing
\(\omega^{+}_{\xi}=-\frac{1}{2}\mbf E\cdot\bs\epsilon_\xi\mbf E\), which gives
\(\mbf d_\xi=\bs\epsilon_\xi\mbf E\).  The field \(\mbf E\) is not a phase field;
the phase index only records that each material phase may have a different
constitutive response to the common spatial field.

The reversible electric-enthalpy power is pulled back with the same phase map
used elsewhere in the virtual-power construction:
%
\begin{align}
	\delta\mc P_{\rm elec}
	={}&
	\sum_\xi\int_{\mc B_{\xi0}}
	\left[
	J_\xi
	\phi_\xi
	\left.
	\frac{\partial\omega^{+}_{\xi}}{\partial\mbf E}
	\right\vert_{\mc X_\xi}
	\cdot\delta\dot{\mbf E}_\xi
	+
	\phi_\xi\omega^{+}_{\xi}\delta\dot J_\xi
	+
	J_\xi
	\left(
	\sum_\alpha\varrho_\xi^\alpha
	\right)
	\delta\dot\varphi
	\right]\dVxi .
	\label{eq:vp_electrical_power_pullback}
\end{align}
%
Using \(\delta\dot J_\xi=J_\xi\nabla_{\mbf x}\cdot\delta\mbf v_\xi\), this
pushes forward to the current form
%
\begin{align}
	\delta\mc P_{\rm elec}
	={}&
...
the virtual phase motion used to pull back the corresponding electrical
enthalpy contribution.  It satisfies
%
\begin{equation}
	\delta\dot{\mbf E}_\xi
	=
	-\nabla_{\mbf x}\delta\dot\varphi
	-
	\left(
	\nabla_{\mbf x}\delta\mbf v_\xi
	\right)^T\mbf E.
	\label{eq:vp_electric_field_variation}
\end{equation}
%
Substituting \eqref{eq:vp_electric_field_variation}, using
\(\mbf d_\xi=-\partial\omega^{+}_{\xi}/\partial\mbf E\) and
\(\mbf d=\sum_\xi\phi_\xi\mbf d_\xi\), and integrating by parts gives the
potential-rate coefficient
%
\begin{align}
...
\end{align}
%
where the reversible electrical, or Maxwell, stress assigned to the phase
pullback is
%
\begin{equation}
	\bs{\sigma}^{+}_{\xi}
...
\subsection{Virtual power of the charge constraint}
...
unit charge, we use the same electric potential \(\varphi\) that appears in
\eqref{eq:electric_field_definitions}.  The constraint power is
%
\begin{align}
	\mc P_{C_5}
	={}&
...
The multiplier variations enforce the original four constraints together with
the charge-projected constraint \(C_5\).  Since \(C_5\) is a charge-weighted
linear combination of the component-rate constraints, it changes the
interpretation of the component multiplier rather than adding an independent
component balance.  The coefficient of the virtual potential rate gives the
electrical Euler--Lagrange equation discussed after the fluid momentum balance.
The phase Maxwell terms in \eqref{eq:vp_collected_principle} are the
integrated-by-parts form of \eqref{eq:vp_electrical_mechanical_power}; their
boundary parts are included with the other boundary powers.
...
\subsection{Electrical field equations}
\label{sec:vp_electrical_field_equations}

The electrical equations follow from the two appearances of the potential
\(\varphi\).  Its multiplier variation in \(C_5\) enforces the charge-projected
component-rate constraint \eqref{eq:vp_C5_charge_constraint}.  This does not add
an independent component balance; it records the charge projection of the
component-rate bookkeeping and gives the component-rate multiplier the
electrochemical form \eqref{eq:vp_electrochemical_potential_definition}.

The independent virtual potential rate \(\delta\dot\varphi\) comes from the
electric enthalpy power.  Its coefficient in
\eqref{eq:vp_collected_principle} gives Gauss' law,
%
\begin{equation}
...
choice of \(\omega^{+}_{\xi}\), or equivalently of \(\mbf d_\xi\), as a
function of electric, mechanical, compositional, and thermal state.  The natural
electrical boundary condition is prescribed normal total displacement
\(\mbf d\cdot\mbf n\), while the essential boundary condition is prescribed
\(\varphi\), up to an arbitrary additive gauge.
...
contributes a reversible Maxwell stress to each phase motion through the
phase-attributed pullback of the phase electric enthalpy.  Its local
contribution appears in \eqref{eq:vp_collected_principle} and in
\eqref{eq:vp_momentum_uniform} as
\(-\nabla_{\mbf x}\cdot\bs{\sigma}^{+}_{\xi}\).  Summing the
phase-attributed stresses recovers the mixture Maxwell stress because
\(\mbf d=\sum_\xi\phi_\xi\mbf d_\xi\).  For a linear dielectric the summed
stress reduces to
\(\mbf E\otimes\mbf d-\frac{1}{2}\left(\mbf E\cdot\mbf d\right)\mbf I\).
Independent polarization would require an additional internal variable and
either a stationarity or dissipation law; those effects are omitted here because
the reservoir-scale model uses a dielectric closure through
\(\omega^{+}_{\xi}\) and \(\mbf d_\xi\).
