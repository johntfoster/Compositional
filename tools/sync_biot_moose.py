...
def push(master: Path, biot: Path) -> None:
    ...
    if set(previous_by_path) != {entry["biot"] for entry in files}:
        raise SyncError("synchronization state does not match the current master manifest")
    ...
    for entry in files:
        ...
        biot_path = biot / entry["biot"]
        if not master_path.is_file() or not biot_path.is_file():
            raise SyncError(f"missing synchronized file: {entry}")
        base_hash = previous_by_path[entry["biot"]]["sha256"]
        master_hash = digest(master_path)
        biot_hash = digest(biot_path)
