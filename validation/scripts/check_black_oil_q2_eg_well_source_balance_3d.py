#!/usr/bin/env python3
"""Check the genuinely 3D Q2/EG black-oil well/source summed Eq. (32) MMS."""
...
DECK = (
    ...
    / "tests"
    / "black_oil_well"
    / "q2_eg_three_component_well_source_balance_3d.i"
)
GOLD = DECK.parent / "gold" / "q2_eg_three_component_well_source_balance_3d.csv"
...
EXPECTED_INTEGRALS = {
    "reference_domain_volume": 24.0,
    "completion_reference_volume": 24.0,
    "solid_reference_jacobian_integral": 1.716 * 24.0,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="black_oil_q2_eg_well_source_mms_3d_") as tmp:
    ...
    if not gold_rows:
        raise SystemExit(f"black-oil Q2/EG well/source gold CSV is empty: {GOLD}")
    gold_final = {name: float(value) for name, value in gold_rows[-1].items()}
    missing_gold = [name for name in required if name not in gold_final]
    if missing_gold:
        raise SystemExit("black-oil Q2/EG well/source gold CSV omitted: " + ", ".join(missing_gold))

    failures = []
    for name in ERROR_COLUMNS:
        if not math.isfinite(final[name]):
            failures.append(f"{name} is not finite")
        elif abs(final[name]) > TOLERANCE:
            failures.append(f"{name}={abs(final[name]):.6e} > {TOLERANCE:.1e}")
    ...
    for name in required:
        if not math.isfinite(gold_final[name]):
            failures.append(f"gold {name} is not finite")
        elif abs(final[name] - gold_final[name]) > TOLERANCE:
            failures.append(
                f"{name}_gold_error={abs(final[name] - gold_final[name]):.6e} "
                f"> {TOLERANCE:.1e}"
            )
    if failures:
        raise SystemExit("3D black-oil Q2/EG well/source MMS failed: " + "; ".join(failures))
    ...
    print(
        "black_oil_q2_eg_well_source_balance_3d: "
        f"maximum_error={maximum_error:.6e}, "
        f"maximum_integral_error={maximum_integral_error:.6e}, "
        "water_oil_gas_source_integrals_expected_observed="
        f"{EXPECTED_INTEGRALS['summed_eq32_water_component_well_source_integral']:.12e}/"
        f"{final['summed_eq32_water_component_well_source_integral']:.12e},"
        f"{EXPECTED_INTEGRALS['summed_eq32_stock_tank_oil_component_well_source_integral']:.12e}/"
        f"{final['summed_eq32_stock_tank_oil_component_well_source_integral']:.12e},"
        f"{EXPECTED_INTEGRALS['summed_eq32_stock_tank_gas_component_well_source_integral']:.12e}/"
