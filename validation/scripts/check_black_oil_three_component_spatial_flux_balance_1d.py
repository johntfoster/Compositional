#!/usr/bin/env python3
"""Quantitatively check the 1D black-oil Q2/EG spatial-flux MMS."""
...
DECK = (
    ...
    / "tests"
    / "black_oil_pvt"
    / "black_oil_three_component_spatial_flux_balance_1d.i"
)
...
EXPECTED_INTEGRALS = {
    "water_left_reference_component_flux": -0.0448 / 1.1,
    "water_right_reference_component_flux": -0.0512 / 1.1,
    "water_net_outward_reference_component_flux": -0.0064 / 1.1,
    "water_reference_component_source_integral": -0.0064 / 1.1,
    "oil_left_reference_component_flux": -0.15 / 1.1,
    "oil_right_reference_component_flux": -0.15 / 1.1,
    "oil_net_outward_reference_component_flux": 0.0,
    "oil_reference_component_source_integral": 0.0,
    "gas_left_reference_component_flux": -0.0391 / 1.1,
    "gas_right_reference_component_flux": -0.04025 / 1.1,
    "gas_net_outward_reference_component_flux": -0.00115 / 1.1,
    "gas_reference_component_source_integral": -0.00115 / 1.1,
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="black_oil_spatial_flux_mms_1d_") as tmp:
    ...
    if not rows:
        raise SystemExit("black-oil spatial-flux MMS produced no CSV rows")
    ...
    if missing:
        raise SystemExit("black-oil spatial-flux MMS omitted: " + ", ".join(missing))
    ...
    if failures:
        raise SystemExit("1D black-oil Q2/EG spatial-flux MMS failed: " + "; ".join(failures))
    ...
    print(
        "black_oil_three_component_spatial_flux_balance_1d: "
