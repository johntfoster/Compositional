#!/usr/bin/env python3
"""Quantitatively check the genuinely 3D black-oil Q2/EG spatial-flux MMS."""
...
DECK = (
    ...
    / "tests"
    / "black_oil_pvt"
    / "black_oil_three_component_spatial_flux_balance_3d.i"
)
...
# F=diag(1.1,1.2,1.3), J=1.716, so the three diagonal entries of
# J F^{-1}F^{-T} are 1.56/1.1, 1.43/1.2, and 1.32/1.3.  For each phase,
# W=-rho k k_r J F^{-1}F^{-T} Grad_X(p).  The direct summed Eq. (32)
# component source is Div_X(sum_xi W_xi eta_xi_component_alpha).
water_x_factor = -2.0 * 0.1 * 0.9 * (1.56 / 1.1)
water_y_factor = -2.0 * 0.1 * 1.92 * (1.43 / 1.2)
water_z_factor = -2.0 * 0.1 * 2.94 * (1.32 / 1.3)
gas_x_factor = -0.1 * 1.09 * (1.56 / 1.1)
gas_y_factor = -0.1 * 2.06 * (1.43 / 1.2)
gas_z_factor = -0.1 * 3.03 * (1.32 / 1.3)
water_divergence = (
    water_x_factor * 0.02 + water_y_factor * 0.016 + water_z_factor * 0.012
)
gas_divergence = (
    gas_x_factor * 0.006 + gas_y_factor * 0.004 + gas_z_factor * 0.002
)

EXPECTED_INTEGRALS = {
    "water_left_reference_component_flux": water_x_factor * (0.28 + 0.016 * 0.5 + 0.012 * 0.5),
    "water_right_reference_component_flux": water_x_factor * (0.28 + 0.02 + 0.016 * 0.5 + 0.012 * 0.5),
    "water_bottom_reference_component_flux": water_y_factor * (0.28 + 0.02 * 0.5 + 0.012 * 0.5),
    "water_top_reference_component_flux": water_y_factor * (0.28 + 0.02 * 0.5 + 0.016 + 0.012 * 0.5),
    "water_back_reference_component_flux": water_z_factor * (0.28 + 0.02 * 0.5 + 0.016 * 0.5),
    "water_front_reference_component_flux": water_z_factor * (0.28 + 0.02 * 0.5 + 0.016 * 0.5 + 0.012),
    ...
    "oil_left_reference_component_flux": -3.0 * 0.1 * (1.56 / 1.1) * 0.5,
    "oil_right_reference_component_flux": -3.0 * 0.1 * (1.56 / 1.1) * 0.5,
    "oil_bottom_reference_component_flux": -3.0 * 0.1 * 2.0 * (1.43 / 1.2) * 0.5,
    "oil_top_reference_component_flux": -3.0 * 0.1 * 2.0 * (1.43 / 1.2) * 0.5,
    "oil_back_reference_component_flux": -3.0 * 0.1 * 3.0 * (1.32 / 1.3) * 0.5,
    "oil_front_reference_component_flux": -3.0 * 0.1 * 3.0 * (1.32 / 1.3) * 0.5,
    ...
    "gas_left_reference_component_flux": gas_x_factor * (0.34 + 0.004 * 0.5 + 0.002 * 0.5),
    "gas_right_reference_component_flux": gas_x_factor * (0.34 + 0.006 + 0.004 * 0.5 + 0.002 * 0.5),
    "gas_bottom_reference_component_flux": gas_y_factor * (0.34 + 0.006 * 0.5 + 0.002 * 0.5),
    "gas_top_reference_component_flux": gas_y_factor * (0.34 + 0.006 * 0.5 + 0.004 + 0.002 * 0.5),
    "gas_back_reference_component_flux": gas_z_factor * (0.34 + 0.006 * 0.5 + 0.004 * 0.5),
    "gas_front_reference_component_flux": gas_z_factor * (0.34 + 0.006 * 0.5 + 0.004 * 0.5 + 0.002),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="black_oil_spatial_flux_mms_3d_") as tmp:
    ...
    if not rows:
        raise SystemExit("3D black-oil spatial-flux MMS produced no CSV rows")
    ...
    if missing:
        raise SystemExit("3D black-oil spatial-flux MMS omitted: " + ", ".join(missing))
    ...
    for component in ("water", "oil", "gas"):
        for side in ("left", "right", "bottom", "top", "back", "front"):
    ...
    if failures:
        raise SystemExit("3D black-oil Q2/EG spatial-flux MMS failed: " + "; ".join(failures))
    ...
    print(
        "black_oil_three_component_spatial_flux_balance_3d: "
