#!/usr/bin/env python3
"""Check the stationary 2D solid-reference thermal energy-balance MMS."""
...
DECK = (
    ...
    / "charged_nonisothermal_energy_balance_2d.i"
)
...
EXPECTED_VALUES = {
    "reference_heat_flux_divergence_integral": -6.05,
    "reference_electric_work_integral": -2.42,
    "reference_heat_supply_integral": -3.63,
    "reference_energy_source_integral": -6.05,
    "left_reference_heat_flux_x": -5.445,
    "right_reference_heat_flux_x": -6.655,
    "bottom_reference_heat_flux_y": -9.68,
    "top_reference_heat_flux_y": -14.52,
    "net_outward_reference_heat_flux": -6.05,
}
...
def run_case(output_base: Path, gauge_shift: float) -> dict[str, float]:
    ...
    if not rows:
        raise SystemExit("2D stationary energy-balance MMS produced no CSV rows")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="charged_nonisothermal_energy_mms_2d_") as tmp:
        tmp_path = Path(tmp)
    ...
    if failures:
        raise SystemExit("2D stationary thermal energy-balance MMS failed: " + "; ".join(failures))
    ...
    print(
        "charged_nonisothermal_energy_balance_mms_2d: "
        ...
        "DivQ_integral=-6.05, electric_work_integral=-2.42, "
        "heat_supply_integral=-3.63, "
        "left_Qx=-5.445, right_Qx=-6.655, "
        "bottom_Qy=-9.68, top_Qy=-14.52, "
