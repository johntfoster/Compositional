#!/usr/bin/env python3
"""Quantitatively check the coupled 2D Q2/EG poromechanics MMS."""
...
DECK = APP_DIR / "test" / "tests" / "eg_hierarchy" / "coupled_q2_eg_poro_mms_2d.i"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="coupled_q2_eg_poro_mms_2d_") as tmp:
    ...
    if not rows:
        raise SystemExit("coupled 2D MMS produced no CSV rows")
    final = {name: float(value) for name, value in rows[-1].items()}
    missing = [name for name in LIMITS if name not in final]
    if missing:
        raise SystemExit("coupled 2D MMS omitted required norms: " + ", ".join(missing))
    failures = [
    ...
    if failures:
        raise SystemExit("coupled 2D Q2/EG poromechanics MMS failed: " + "; ".join(failures))

    print(
        "coupled_q2_eg_poro_mms_2d: "
