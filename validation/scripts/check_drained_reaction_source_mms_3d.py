#!/usr/bin/env python3
"""Quantitatively check the 3D TET10 Q2/EG reaction/source MMS."""
...
DECK = APP_DIR / "test" / "tests" / "reaction_tau" / "drained_reaction_source_mms_3d.i"
...
EXPECTED_INTEGRALS = {
    ...
    "net_outward_reference_component_flux": -0.010875,
    "summed_reference_component_source_integral": 0.001625,
    # At t=0.2, integrate each affine flux component over its unit face.
    "left_reference_component_flux_x": -0.15435,
    "right_reference_component_flux_x": -0.16035,
    "bottom_reference_component_flux_y": -0.116325,
    "top_reference_component_flux_y": -0.1197,
    "back_reference_component_flux_z": -0.077925,
    "front_reference_component_flux_z": -0.079425,
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="drained_reaction_source_mms_3d_") as tmp:
    ...
    if not rows:
        raise SystemExit("3D reaction/source MMS produced no CSV rows")
    ...
    if missing:
        raise SystemExit("3D reaction/source MMS omitted: " + ", ".join(missing))
    ...
    if abs(final["net_outward_reference_component_flux"]) <= TOLERANCE:
        failures.append("net outward reference-component flux is zero")
    if failures:
        raise SystemExit("3D TET10 Q2/EG reaction/source MMS failed: " + "; ".join(failures))

    maximum_error = max(
        max(abs(final[name]) for name in ERROR_COLUMNS),
        balance,
        max(integral_errors.values()),
    )
    print(
        "drained_reaction_source_mms_3d: "
        f"maximum_error={maximum_error:.6e}, global_balance={balance:.6e}, "
        "storage_rate=0.0125, net_outward_flux=-0.010875, source=0.001625, "
